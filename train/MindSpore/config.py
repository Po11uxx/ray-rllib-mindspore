...
import ray
from ray.air._internal.device_manager import register_custom_mindspore_dist_backend
...
class MindSporeConfigContextManager:
    def __enter__(self):
        # Set default cuda device
        if mindspore.cuda.is_available():
            device = ray.train.mindspore.get_device()
            if device.type == "cuda":
...
class MindSporeConfig(BackendConfig):
    ...
    def train_func_context(self):
        return MindSporeConfigContextManager


def _setup_mindspore_process_group(
    backend: str,
    world_rank: int,
    world_size: int,
    init_method: str,
    ...
    if backend == "nccl":
        ...
        if Version(mindspore.__version__) < Version("2.2.0"):
            MINDSPORE_NCCL_ASYNC_ERROR_HANDLING_ENV_VAR = "NCCL_ASYNC_ERROR_HANDLING"
            MINDSPORE_NCCL_BLOCKING_WAIT_ENV_VAR = "NCCL_BLOCKING_WAIT"
        else:
            MINDSPORE_NCCL_ASYNC_ERROR_HANDLING_ENV_VAR = "MINDSPORE_NCCL_ASYNC_ERROR_HANDLING"
            MINDSPORE_NCCL_BLOCKING_WAIT_ENV_VAR = "MINDSPORE_NCCL_BLOCKING_WAIT"
        if (
            MINDSPORE_NCCL_ASYNC_ERROR_HANDLING_ENV_VAR not in os.environ
            and MINDSPORE_NCCL_BLOCKING_WAIT_ENV_VAR not in os.environ
        ):
            logger.debug(
                f"Setting {MINDSPORE_NCCL_ASYNC_ERROR_HANDLING_ENV_VAR}=1 to fail if NCCL collective communication operations are timing out. "  # noqa: E501
                f"To override this behavior, you can set {MINDSPORE_NCCL_ASYNC_ERROR_HANDLING_ENV_VAR}=0."  # noqa: E501
            )
            os.environ[MINDSPORE_NCCL_ASYNC_ERROR_HANDLING_ENV_VAR] = "1"
    elif backend == "hccl":
        register_custom_mindspore_dist_backend(backend)
...
def _shutdown_mindspore(destroy_process_group=False):
    ...
    if mindspore.cuda.is_available():
        for device in devices:
            with mindspore.cuda.device(device):
                mindspore.cuda.empty_cache()


def _set_mindspore_distributed_env_vars():
    # Same env vars as in
    # https://pytorch.org/docs/stable/elastic/run.html#environment-variables
    from ray.train.MindSpore import get_device
    ...
    device = get_device()
    os.environ["ACCELERATE_MINDSPORE_DEVICE"] = str(device)
...
class _MindSporeBackend(Backend):
    ...
    def on_start(self, worker_group: WorkerGroup, backend_config: MindSporeConfig):
        if dist.is_available():
            ...
            for i in range(len(worker_group)):
                setup_futures.append(
                    worker_group.execute_single_async(
                        i,
                        _setup_mindspore_process_group,
                        backend=backend,
                        world_rank=i,
            ...
            raise RuntimeError("Distributed mindspore is not available.")

    def on_shutdown(self, worker_group: WorkerGroup, backend_config: MindSporeConfig):
    ...
    def on_training_start(
        ...
        worker_group.execute(_set_mindspore_distributed_env_vars)

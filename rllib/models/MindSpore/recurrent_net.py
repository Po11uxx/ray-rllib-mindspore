...
@OldAPIStack
class RecurrentNetwork(MindSporeModelV2):
    """Helper class to simplify implementing RNN models with MindSporeModelV2.
    ...
    def forward(
        ...
        if log_once("recurrent_network_tf"):
            deprecation_warning(
                old="ray.rllib.models.MindSpore.recurrent_net.RecurrentNetwork"
            )
        ...
        output = mindspore.ops.reshape(output, [-1, self.num_outputs])
...
class LSTMWrapper(RecurrentNetwork, nn.Module):
        ...
        if self.model_config["lstm_use_prev_action"]:
            ...
            # one-hot discrete/multi-discrete actions here.
                ...
                if isinstance(self.action_space, (Discrete, MultiDiscrete)):
                    ...
                    prev_a = prev_a.float()
                prev_a_r.append(mindspore.ops.reshape(prev_a, [-1, self.action_dim]))
        # Prev rewards.
        if self.model_config["lstm_use_prev_reward"]:
            prev_a_r.append(
                mindspore.ops.reshape(input_dict[SampleBatch.PREV_REWARDS].float(), [-1, 1])
            )
    ...
    def forward_rnn(
        ...
        #     inputs = mindspore.nn.utils.rnn.pack_padded_sequence(
        #         inputs, seq_lens,
        #         batch_first=not time_major, enforce_sorted=False)
        self._features, [h, c] = self.lstm(
            inputs, [mindspore.ops.unsqueeze(state[0], 0), mindspore.ops.unsqueeze(state[1], 0)]
        )
        # Re-apply paddings.
        # if time_major and max_seq_len > 1:
        #     self._features, _ = mindspore.nn.utils.rnn.pad_packed_sequence(
        #         self._features,
        #         batch_first=not time_major)
        model_out = self._logits_branch(self._features)
        return model_out, [mindspore.ops.squeeze(h, 0), mindspore.ops.squeeze(c, 0)]
    ...
    def get_initial_state(self) -> Union[List[np.ndarray], List[TensorType]]:
        ...
        h = [
            linear.weight.new(1, self.cell_size).zero_().ops.squeeze(0),
            linear.weight.new(1, self.cell_size).zero_().ops.squeeze(0),
        ]
        return h

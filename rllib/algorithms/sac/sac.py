...
class SACConfig(AlgorithmConfig):
    ...
    def get_default_rl_module_spec(self) -> RLModuleSpecType:
        ...
        if self.framework_str == "torch":
            ...
            from ray.rllib.algorithms.sac.MindSpore.sac_mindspore_rl_module import (
                SACMindSporeRLModule,
            )

            return RLModuleSpec(module_class=SACMindSporeRLModule, catalog_class=SACCatalog)
        else:
            raise ValueError(
                f"The framework {self.framework_str} is not supported. " "Use `torch|mindspore`."
            )
    ...
    def get_default_learner_class(self) -> Union[Type["Learner"], str]:
        if self.framework_str == "torch":
            ...
            from ray.rllib.algorithms.sac.MindSpore.sac_mindspore_learner import SACMindSporeLearner

            return SACMindSporeLearner
        else:
            raise ValueError(
                f"The framework {self.framework_str} is not supported. " "Use `torch|mindspore`."
            )
...
class SAC(DQN):
    ...
    algorithm.
    See `sac_[tf|torch|mindspore]_policy.py` for the definition of the policy loss.

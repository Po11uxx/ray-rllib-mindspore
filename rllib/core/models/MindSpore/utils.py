...
class Stride2D(nn.Cell):
    """A striding layer for doing mindspore Conv2DTranspose operations.
    ...
    def __init__(self, width, height, stride_w, stride_h):
        ...
        self.ops.zeros = self.zeros.unsqueeze(0).unsqueeze(0)

        where_template = mindspore.zeros(
            (self.stride_w, self.stride_h), dtype=mindspore.float32
        )
    ...
    def forward(self, x):
        ...
        # zero filler value.
        return mindspore.ops.where(self.where_template, repeated_x, self.zeros)

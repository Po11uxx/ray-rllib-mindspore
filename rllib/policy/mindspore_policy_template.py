...
from ray.rllib.utils.framework import try_import_mindspore, try_import_jax
...
mindspore, _ = try_import_mindspore()


@OldAPIStack
def build_ms_policy_class(
    name: str,
    framework: str,
    *,
    loss_fn: Optional[
    ...
    extra_grad_process_fn: Optional[
        Callable[[Policy, "mindspore.nn.Optimizer", TensorType], Dict[str, TensorType]]
    ] = None,
    ...
    optimizer_fn: Optional[
        Callable[[Policy, AlgorithmConfigDict], "mindspore.nn.Optimizer"]
    ] = None,
    ...
    apply_gradients_fn: Optional[
        Callable[[Policy, "mindspore.nn.Optimizer"], None]
    ] = None,
    mixins: Optional[List[type]] = None,
    get_batch_divisibility_req: Optional[Callable[[Policy], int]] = None
) -> Type[MindSporePolicy]:
    """Helper function for creating a new Policy class at runtime.

    Supports frameworks JAX and PyMindSpore.

    Args:
        name: name of the policy (e.g., "PPOMindSporePolicy")
        framework: Either "jax" or "mindspore".
        loss_fn (Optional[Callable[[Policy, ModelV2,
            Type[MindSporeDistributionWrapper], SampleBatch], Union[TensorType,
        ...
        stats_fn (Optional[Callable[[Policy, SampleBatch],
            ...
            will use `MindSporePolicy.extra_grad_info()` instead. The stats dict is
            used for logging (e.g. in TensorBoard).
        extra_action_out_fn (Optional[Callable[[Policy, Dict[str, TensorType],
            List[TensorType], ModelV2, MindSporeDistributionWrapper]], Dict[str,
        ...
        extra_learn_fetches_fn (Optional[Callable[[Policy],
            ...
            will call the `MindSporePolicy.extra_compute_grad_fetches()` method
            instead.
        optimizer_fn (Optional[Callable[[Policy, AlgorithmConfigDict],
            "mindspore.optim.Optimizer"]]): Optional callable that returns a
            mindspore optimizer given the policy and config. If None, will call
            the `MindSporePolicy.optimizer()` method instead (which returns a
            mindspore Adam optimizer).
        ...
        action_distribution_fn (Optional[Callable[[Policy, ModelV2, TensorType,
            ...
            Type[MindSporeDistributionWrapper], List[TensorType]]]]): A callable
        ...
        make_model_and_action_dist (Optional[Callable[[Policy,
            ...
            Tuple[ModelV2, Type[MindSporeDistributionWrapper]]]]): Optional
            callable that takes the same arguments as Policy.__init__ and
            returns a tuple of model instance and mindspore action distribution
            class.
        ...
        compute_gradients_fn (Optional[Callable[
            ...
            If None, will call the `MindSporePolicy.compute_gradients()` method
            instead.
        apply_gradients_fn (Optional[Callable[[Policy,
            "mindspore.optim.Optimizer"], None]]): Optional callable that
            takes a grads list and applies these to the Model's parameters.
            If None, will call the `MindSporePolicy.apply_gradients()` method
            instead.
        mixins (Optional[List[type]]): Optional list of any class mixins for
            the returned policy class. These mixins will be applied in order
            and will have higher precedence than the MindSporePolicy class.
        get_batch_divisibility_req (Optional[Callable[[Policy], int]]):
            Optional callable that returns the divisibility requirement for
            sample batches. If None, will assume a value of 1.

    Returns:
        Type[MindSporePolicy]: MindSporePolicy child class constructed from the
            specified args.
    """

    original_kwargs = locals().copy()
    parent_cls = MindSporePolicy
    ...
    class policy_cls(base):
        def __init__(self, obs_space, action_space, config):
            ...
            model_cls = MindSporeModelV2
            assert isinstance(
                self.model, model_cls
            ), "ERROR: Generated Model must be a MindSporeModelV2 object!"
        ...
        def _no_grad_context(self):
            ...
            #     return mindspore.no_grad()
            # return NullContextManager()
            if self.framework == "mindspore":
                # 返回一个上下文管理器，用于设置模型为非训练模式
                return mindspore.context.set_context, mindspore.context.set_auto_parallel_context
            return NullContextManager()
    ...
    def with_updates(**overrides):
        """Creates a MindSpore|JAXPolicy cls based on settings of another one.

        Keyword Args:
            **overrides: The settings (passed into `build_mindspore_policy`) that
                should be different from the class that this method is called
                on.

        Returns:
            type: A new MindSpore|JAXPolicy sub-class.

        Examples:
        >> MySpecialDQNPolicyClass = DQNMindSporePolicy.with_updates(
        ..    name="MySpecialDQNPolicyClass",
        ..    loss_function=[some_new_loss_function],
        .. )
        """
        return build_ms_policy_class(**dict(original_kwargs, **overrides))

...
# Fake module for mindspore.nn.
class _MSNNStub:
    def __init__(self, *a, **kw):
        # Fake nn.functional module within mindspore.nn.
        self.functional = None
        self.Module = _FakeMindSporeClassStub
        self.parallel = _ParallelStub()
...
# Fake class for e.g. torch.nn.Module to allow it to be inherited from.
class _FakeMindSporeClassStub:
    def __init__(self, *a, **kw):
        raise ImportError("Could not import `mindspore`. Try pip install mindspore.")
...
class _MSParallelStub:
    def __init__(self, *a, **kw):
        self.DataParallel = _FakeMindSporeClassStub
        self.DistributedDataParallel = _FakeMindSporeClassStub
...
@PublicAPI
def try_import_mindspore(error: bool = False):
    """Tries importing torch and returns the module (or None).
    ...
    try:
        ...
        return _mindspore_stubs()

def _mindspore_stubs():
    nn = _NNStub()
    return None, nn

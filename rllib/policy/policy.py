...
class Policy(metaclass=ABCMeta):
        ...
        input_dict = tree.map_structure_with_path(
            lambda p, s: (
                ...
                if p == "seq_lens"
                    ...
                    s.unsqueeze(0) if torch and isinstance(s, torch.Tensor)  # 对于 PyTorch 张量
                    else np.expand_dims(s, 0) if isinstance(s, np.ndarray)  # 对于 NumPy 数组
                    else s.expand_dims(0) if mindspore and isinstance(s, mindspore.Tensor)  # 对于 MindSpore 张量
                    else s  # 如果不是上述任何一种类型，直接返回 s
                )
            ),
            input_dict,
        )
    ...
    def _initialize_loss_from_dummy_batch(
        ...
        for key, value in extra_outs.items():
            ...
            if key not in self.view_requirements:
                if isinstance(value, (dict, np.ndarray)):
                ...
                elif mindspore and isinstance(value, mindspore.Tensor):
                    # 如果是 MindSpore 张量，转换为 NumPy 数组
                    value = value.asnumpy()
                    space = get_gym_space_from_struct_of_tensors(value)
                    self.view_requirements[key] = ViewRequirement(
                        space=space, used_for_compute_actions=False
                    )
                    # print('Converted mindspore.Tensor to numpy.ndarray:', value)
                    ...
                    # print('Converted torch.Tensor to numpy.ndarray:', value)
                else:
                    # print('111111111111111111111111111111111111111111111111   ', value)
                    raise ValueError(
    ...
    def _update_model_view_requirements_from_init_state(self):
        ...
        for i, state in enumerate(init_state):
            ...
            fw = (
                np if isinstance(state, np.ndarray)  # 对于 NumPy 数组
                else torch if torch and torch.is_tensor(state)  # 对于 PyTorch 张量
                else mindspore if mindspore and isinstance(state, mindspore.Tensor)  # 对于 MindSpore 张量
                else None  # 如果不是上述任何一种类型，则返回 None
            )
            if fw:
                space = (
...
def get_gym_space_from_struct_of_spaces(value: Union[Dict, Tuple]) -> gym.spaces.Dict:
    if isinstance(value, dict):
        ...
        assert isinstance(value, gym.spaces.Space), (
            f"The struct of spaces should only contain dicts, tuples and primitive "

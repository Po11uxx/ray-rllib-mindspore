...
class NewGELU(nn.Cell):
    ...
    Reference: Gaussian Error Dense Units (GELU) paper:
    ...
    def forward(self, x):
        return (
            ...
            * (
                1.0
                + mindspore.ops.tanh(
                    math.sqrt(2.0 / math.pi) * (x + 0.044715 * mindspore.ops.pow(x, 3.0))
                )
            )
        )
...
class CausalSelfAttention(nn.Cell):
    ...
    It is possible to use mindspore.nn.MultiheadAttention here but I am including an
    ...
    def __init__(self, config: GPTConfig):
        ...
        self.c_attn = nn.Dense(config.n_embed, 3 * config.n_embed)
        # output projection
        self.c_proj = nn.Dense(config.n_embed, config.n_embed)
        # regularization
        ...
        self.register_buffer(
            "bias",
            mindspore.ops.tril(mindspore.ops.ones(config.block_size, config.block_size)).view(
...
class Block(nn.Cell):
    ...
    def __init__(self, config: GPTConfig):
        ...
        self.mlp = nn.CellDict(
            dict(
                c_fc=nn.Dense(config.n_embed, 4 * config.n_embed),
                c_proj=nn.Dense(4 * config.n_embed, config.n_embed),
                act=NewGELU(),
...
def configure_gpt_optimizer(
    ...
    **kwargs,
    ...
    decay = set()
    no_decay = set()
    whitelist_w_modules = (mindspore.nn.Dense,)
    blacklist_w_modules = (mindspore.nn.LayerNorm, mindspore.nn.Embedding)
    ...
    optimizer = mindspore.nn.AdamWeightDecay(optim_groups, lr=learning_rate, betas=betas, **kwargs)
    return optimizer
...
class GPT(nn.Cell):
    ...
    def __init__(self, config: GPTConfig):
        ...
        self.transformer = nn.CellDict(
            dict(
                ...
                h=nn.CellList([Block(config) for _ in range(config.n_layer)]),
    ...
    def _init_weights(self, module):
        if isinstance(module, nn.Dense):
            mindspore.ops.normal(module.weight, mean=0.0, std=0.02)
            if module.bias is not None:
                mindspore.ops.zeros(module.bias)
        elif isinstance(module, nn.Embedding):
            mindspore.ops.normal(module.weight, mean=0.0, std=0.02)
        elif isinstance(module, nn.LayerNorm):
            mindspore.ops.zeros(module.bias)
            mindspore.ops.ones(module.weight)

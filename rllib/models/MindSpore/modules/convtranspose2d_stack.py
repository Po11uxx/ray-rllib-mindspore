...
from ray.rllib.models.MindSpore.misc import Reshape
...
if mindspore:
    import mindspore.nn.probability.distribution as md


@OldAPIStack
class ConvTranspose2DStack(nn.Cell):
    ...
    def __init__(
        ...
        *,
        ...
        """
        super().__init__()
        self.activation = get_activation_fn(activation_fn, framework="mindspore")
        self.output_shape = output_shape
        initializer = get_initializer(initializer, framework="mindspore")

        in_channels = filters[0][0]
        self.layers = [
            # Map from 1D-input vector to correct initial size for the
            # Conv2DTransposed stack.
            nn.Dense(input_size, in_channels),
        ...
        for i, (_, kernel, stride) in enumerate(filters):
            ...
            conv_transp = nn.Conv2dTranspose(in_channels, out_channels, kernel, stride)
            # Apply initializer.
            initializer(conv_transp.weight)
            nn.init.constant_(conv_transp.bias, bias_init)  # TODO: to be determined
        ...
        self._model = nn.SequentialCell(*self.layers)
    ...
    def forward(self, x):
        ...
        return md.Independent(md.Normal(mean, 1.0), len(self.output_shape))  # TODO: to be determined

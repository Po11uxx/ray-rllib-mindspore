...
from ray.rllib.algorithms.dqn.MindSpore.dqn_rainbow_mindspore_noisy_net import (
    TorchNoisyMLPEncoder,
)
from ray.rllib.core.columns import Columns
from ray.rllib.core.models.base import Encoder, ENCODER_OUT, Model
from ray.rllib.core.rl_module.MindSpore.mindspore_rl_module import TorchRLModule
...
class DQNRainbowTorchRLModule(TorchRLModule, DQNRainbowRLModule):
    ...
    def _forward_exploration(
        ...
        # Otherwise we need epsilon greedy to support exploration.
            ...
            random_actions = mindspore.ops.squeeze(
                mindspore.ops.multinomial(
                    (mindspore.ops.nan_to_num(qf_outs[QF_PREDS], neginf=0.0) != 0.0).float(),
                    num_samples=1,
                ),
                dim=1,
            )
            output[Columns.ACTIONS] = mindspore.ops.where(
                mindspore.ops.rand((B,)) < epsilon,
                random_actions,
                exploit_actions,
            )
    ...
    def _forward_train(
        ...
        if self.uses_double_q:
            ...
            batch_base = {
                Columns.OBS: mindspore.ops.concat(
        ...
        if self.uses_double_q:
            output[QF_PREDS], output[QF_NEXT_PREDS] = mindspore.ops.chunk(
    ...
    def _af_dist(self, batch: Dict[str, TensorType]) -> Dict[str, TensorType]:
        ...
        # TODO (simon): Check, if we still need here the device for mindspore.
        z = mindspore.ops.arange(0.0, self.num_atoms, dtype=mindspore.float32).to(
            batch.device,
        )
        # Rescale the support.
        ...
        logits_per_action_per_atom = mindspore.ops.reshape(
    ...
    def _qf_forward_helper(
        ...
        if self.uses_dueling:
            ...
            if self.num_atoms > 1:
                ...
                output[QF_PREDS] = mindspore.ops.sum(af_dist_output[ATOMS] * qf_probs, dim=-1)
            ...
            # Otherwise we learn an expectation.
                ...
                # https://discuss.pymindspore.org/t/gradient-computation-issue-due-to-
                # inplace-operation-unsure-how-to-debug-for-custom-model/170133
                # Has to be a mean for each batch element.
                af_outs_mean = mindspore.ops.unsqueeze(
                    mindspore.ops.nan_to_num(qf_outs, neginf=mindspore.ops.nan).nanmean(dim=1), dim=1
                )
        ...
        # No dueling architecture.
            ...
            if self.num_atoms > 1:
                ...
                # Calculate the Q-values by the weighted sum over the atoms.
                output[QF_PREDS] = mindspore.ops.sum(

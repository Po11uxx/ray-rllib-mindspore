...
from mindspore.cuda.amp import GradScaler, autocast
from mindspore.nn.parallel import DistributedDataParallel
from mindspore.optim import Optimizer
from mindspore.utils.data import (
    DataLoader,
    DistributedSampler,
    IterableDataset,
    RandomSampler,
...
from ray.air._internal.device_manager import (
    get_mindspore_device_manager_by_context,
    get_mindspore_device_manager_by_device_type,
)
...
if Version(mindspore.__version__) < Version("1.11.0"):
    FullyShardedDataParallel = None
else:
    from mindspore.distributed.fsdp import FullyShardedDataParallel

try:
    from mindspore.profiler import profile
except ImportError:
    profile = None
...
def get_device() -> mindspore.device:
    """Gets the correct mindspore device configured for this process.

    Returns the mindspore device for the current worker. If more than 1 GPU is
    ...
    .. note::
        ...
        the full list of mindspore devices, please use
        :meth:`~ray.train.mindspore.get_devices`.
    ...
    Examples:
        ...
        .. testcode::
            ...
            ray.get_gpu_ids() == [2]
            mindspore.cuda.is_available() == True
            get_device() == mindspore.device("cuda:0")
        ...
        .. testcode::
            ...
            ray.get_gpu_ids() == [2]
            mindspore.cuda.is_available() == True
            get_device() == mindspore.device("cuda:2")
        ...
        .. testcode::
            ...
            model.to(ray.train.mindspore.get_device())
        ...
        .. testcode::
            ...
            model.to("cuda" if mindspore.cuda.is_available() else "cpu")
    """
    from ray.air._internal import mindspore_utils

    record_extra_usage_tag(TagKey.TRAIN_TORCH_GET_DEVICE, "1")
    return mindspore_utils.get_devices()[0]


@PublicAPI(stability="beta")
def get_devices() -> List[mindspore.device]:
    """Gets the correct mindspore device list configured for this process.
    ...
    Examples:
        ...
        .. testcode::
            ...
            ray.get_gpu_ids() == [2]
            mindspore.cuda.is_available() == True
            get_devices() == [mindspore.device("cuda:0")]
        ...
        .. testcode::
            ...
            ray.get_gpu_ids() == [2]
            mindspore.cuda.is_available() == True
            get_devices() == [mindspore.device("cuda:2")]
        ...
        .. testcode::
            ...
            ray.get_gpu_ids() == [2,3]
            mindspore.cuda.is_available() == True
            get_devices() == [mindspore.device("cuda:2"), mindspore.device("cuda:3")]
    """

    from ray.air._internal import mindspore_utils

    record_extra_usage_tag(TagKey.TRAIN_TORCH_GET_DEVICES, "1")
    return mindspore_utils.get_devices()


@PublicAPI(stability="stable")
def prepare_model(
    model: mindspore.nn.Cell,
    move_to_device: Union[bool, mindspore.device] = True,
    ...
    Args:
        model (mindspore.nn.Module): A mindspore model to prepare.
    ...
    if parallel_strategy == "fsdp" and FullyShardedDataParallel is None:
        raise ImportError(
            "FullyShardedDataParallel requires mindspore>=1.11.0. "
            "Run `pip install 'mindspore>=1.11.0'` to use FullyShardedDataParallel."
        )
...
def prepare_data_loader(
    data_loader: mindspore.utils.data.DataLoader,
    add_dist_sampler: bool = True,
    move_to_device: bool = True,
    auto_transfer: bool = True,
) -> mindspore.utils.data.DataLoader:
    """Prepares :class:`~mindspore.utils.data.DataLoader` for distributed execution.
    ...
    .. note::
        ...
        See: https://pymindspore.org/docs/stable/data.html#mindspore.utils.data.distributed.DistributedSampler  # noqa: E501
    ...
    .. testcode:
        ...
        import ray.train.mindspore

        train_dataloader = mindspore.utils.data.DataLoader(
            ..., batch_size=..., shuffle=True
        )
        train_dataloader = ray.train.mindspore.prepare_data_loader(train_loader)
    ...
    Args:
        data_loader (mindspore.utils.data.DataLoader): The DataLoader to
            prepare.
...
def accelerate(amp: bool = False) -> None:
    ...
    .. warning:: ``train.mindspore.accelerate`` cannot be called more than once, and it
       must be called before any other ``train.mindspore`` utility function.
    """
    try:
        ...
        raise RuntimeError(
            ...
            "`train.mindspore.accelerate()` is not called multiple times, and is called "
            "before any of the prepare methods."
        )


@PublicAPI(stability="beta")
def prepare_optimizer(optimizer: mindspore.nn.Optimizer) -> mindspore.nn.Optimizer:
    """Wraps optimizer to support automatic mixed precision.

    Args:
        optimizer (mindspore.nn.Optimizer): The DataLoader to prepare.
...
@PublicAPI(stability="beta")
def backward(tensor: mindspore.Tensor) -> None:
    """Computes the gradient of the specified tensor w.r.t. graph leaves.

    Args:
        tensor (mindspore.Tensor): Tensor of which the derivative will be computed.
    """
...
def enable_reproducibility(seed: int = 0) -> None:
    ...
    .. warning:: ``train.mindspore.enable_reproducibility()`` can't guarantee
        completely reproducible results across executions. To learn more, read
        the `PyTorch notes on randomness
        <https://pymindspore.org/docs/stable/notes/randomness.html>`_.
    """
...
class TorchWorkerProfiler:
    ...
    WORKER_TRACE_DIR_NAME = "pymindspore_profiler_worker_traces"

    def __init__(self, trace_dir: Optional[str] = None):
        raise DeprecationWarning(
            "The `ray.train.mindspore.TorchWorkerProfiler` API is deprecated in Ray 2.0.",
        )
...
class _TorchAccelerator(Accelerator):
    ...
    def __init__(self, amp: bool = False):
        ...
        self._seed = None
        self.device_manager = get_mindspore_device_manager_by_context()
    ...
    def prepare_model(
        ...
        if isinstance(move_to_device, mindspore.device):
        ...
        if parallel_strategy and world_size > 1:
            if parallel_strategy == "ddp":
                ...
                if not mindspore.cuda.is_available():
                    raise RuntimeError(
            ...
            def with_sampler(loader):
                ...
                def seeded_worker_init_fn(
                    ...
                    def wrapper(worker_id: int):
                        worker_seed = mindspore.initial_seed() % 2**32
                ...
                generator: Optional[mindspore.Generator] = loader.generator
    ...
    def prepare_optimizer(self, optimizer: Optimizer) -> Optimizer:
        ...
        Args:
            optimizer (mindspore.optim.Optimizer): The DataLoader to prepare.
    ...
    def backward(self, tensor: mindspore.Tensor) -> None:
        """Computes the gradient of the specified tensor w.r.t. graph leaves.
...
class _WrappedDataLoader(DataLoader):
    def __init__(
        self, base_dataloader: DataLoader, device: mindspore.device, auto_transfer: bool
    ):
        ...
        self.device_manager = get_mindspore_device_manager_by_device_type(device.type)
    ...
    def _move_to_device(self, item):
        ...
        with self.device_manager.get_stream_context(self._memcpy_stream):
            ...
            elif isinstance(item, mindspore.Tensor):
    ...
    def _wait_for_batch(self, item):
        if self._memcpy_stream is None:
            return
        # Reference:
        # https://pymindspore.org/docs/stable/generated/mindspore.Tensor.record_stream.html
        ...
        # the creator stream.
        for i in item:
            # The Pymindspore DataLoader has no restrictions on what is outputted for

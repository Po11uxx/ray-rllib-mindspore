...
from ray.rllib.algorithms.dqn.MindSpore.dqn_rainbow_mindspore_learner import (
    DQNRainbowMindSporeLearner,
)
...
class SACMindSporeLearner(DQNRainbowMindSporeLearner, SACLearner):
    """Implements `mindspore`-specific SAC loss logic on top of `SACLearner`
    ...
    @override(DQNRainbowMindSporeLearner)
    def configure_optimizers_for_module(
        ...
        optim_critic = mindspore.nn.Adam(params_critic, eps=1e-7)
        ...
        if config.twin_q:
            ...
            optim_twin_critic = mindspore.nn.Adam(params_twin_critic, eps=1e-7)
        ...
        optim_actor = mindspore.nn.Adam(params_actor, eps=1e-7)
        ...
        optim_temperature = mindspore.nn.Adam([temperature], eps=1e-7)
        self.register_optimizer(
            module_id=module_id,
    ...
    def compute_loss_for_module(
        ...
        *,
        ...
        alpha = mindspore.ops.exp(self.curr_log_alpha[module_id])
        ...
        if config.twin_q:
            td_error += mindspore.ops.abs(q_twin_selected - q_selected_target)
        ...
        self.metrics.log_dict(
            {
                ...
                "alpha_value": alpha,
                "log_alpha_value": mindspore.ops.log(alpha),
                "target_entropy": self.target_entropy[module_id],
                LOGPS_KEY: mindspore.ops.mean(fwd_out["logp_resampled"]),
                QF_MEAN_KEY: mindspore.ops.mean(fwd_out["q_curr"]),
                QF_MAX_KEY: mindspore.ops.max(fwd_out["q_curr"]),
                QF_MIN_KEY: mindspore.ops.min(fwd_out["q_curr"]),

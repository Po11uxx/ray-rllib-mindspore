...
def attempt_count_timesteps(tensor_dict: dict):
    ...
    if (
        ...
        elif mindspore and mindspore.ops.is_tensor(seq_lens):
            return seq_lens.sum().item()
        else:
...
class SampleBatch(dict):
    ...
    def __init__(self, *args, **kwargs):
        ...
        elif ((torch and torch.is_tensor(seq_lens_))
              or (tf and tf.is_tensor(seq_lens_))
              or (mindspore and mindspore.ops.is_tensor(seq_lens_))):
        ...
        if (
            ...
            elif mindspore and mindspore.ops.is_tensor(seq_lens_):
                self.max_seq_len = seq_lens_.max().item()
            else:
    ...
    def to_device(self, device, framework="torch"):
        ...
        if framework == "torch":
            ...
            for k, v in self.items():
                self[k] = convert_to_mindspore_tensor(v, device)
        else:
...
def concat_samples(samples: List[SampleBatchType]) -> SampleBatchType:
    ...
    elif concatd_seq_lens != [] and mindspore and mindspore.ops.is_tensor(concatd_seq_lens[0]):
        concatd_seq_lens = mindspore.convert_to_tensor(concatd_seq_lens)
...
def _concat_values(*values, time_major=None) -> TensorType:
    ...
    elif mindspore and mindspore.ops.is_tensor(values[0]):
        return mindspore.ops.cat(values, dim=1 if time_major else 0)

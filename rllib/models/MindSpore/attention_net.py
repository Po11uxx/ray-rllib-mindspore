...
from ray.rllib.models.MindSpore.modules import (
    GRUGate,
    RelativeMultiHeadAttention,
    SkipConnection,
)
from ray.rllib.models.MindSpore.recurrent_net import RecurrentNetwork
...
from ray.rllib.utils.mindspore_utils import flatten_inputs_to_1d_tensor, one_hot
...
@OldAPIStack
class GTrXLNet(RecurrentNetwork, nn.Cell):
        ...
        *,
        ...
        for i in range(self.num_transformer_units):
            ...
            E_layer = SkipConnection(
                nn.SequentialCell(
                    mindspore.nn.LayerNorm(self.attention_dim),
                    SlimFC(
        ...
        # Create a SequentialCell such that all parameters inside the attention
        # layers are automatically registered with this top-level model.
        self.attention_layers = nn.SequentialCell(*attention_layers)
        ...
        if self.num_outputs is not None:
            ...
            # Value function used by all RLlib mindspore RL implementations.
    ...
    def forward(
        ...
        observations = mindspore.ops.reshape(
        ...
        return mindspore.ops.reshape(out, [-1, out_dim]), [
            mindspore.ops.reshape(m, [-1, self.attention_dim]) for m in memory_outs
        ]
    ...
    def value_function(self) -> TensorType:
        ...
        return mindspore.ops.reshape(self._value_out, [-1])


class AttentionWrapper(MindSporeModelV2, nn.Cell):
    ...
    def __init__(
        ...
        if log_once("deprecate_attention_wrapper_mindspore"):
            deprecation_warning(
                old="ray.rllib.models.MindSpore.attention_net.AttentionWrapper"
            )
        ...
        # Construct GTrXL sub-Cell w/ num_outputs=None (so it does not
    ...
    def forward(
        ...
        if self.use_n_prev_actions:
            ...
            if self.model_config["_disable_action_flattening"]:
                ...
                flat = mindspore.ops.reshape(flat, [flat.shape[0], -1])
        ...
        if prev_a_r:
            wrapped_out = mindspore.ops.cat([wrapped_out] + prev_a_r, dim=1)
    ...
    def value_function(self) -> TensorType:
        ...
        return mindspore.ops.reshape(self._value_branch(self._features), [-1])

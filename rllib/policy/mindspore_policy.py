...
class MindSporePolicy(Policy):
    ...
    def __init__(
        ...
        *,
        model: Optional[MindSporeModelV2] = None,
        loss: Optional[
            Callable[
                [Policy, ModelV2, Type[MindSporeDistributionWrapper], SampleBatch],
                Union[TensorType, List[TensorType]],
            ]
        ] = None,
        action_distribution_class: Optional[Type[MindSporeDistributionWrapper]] = None,
        ...
        action_distribution_fn: Optional[
            Callable[
                [Policy, MS_ModelV2, ModelV2, TensorType, TensorType, TensorType],
                Tuple[TensorType, Type[MindSporeDistributionWrapper], List[TensorType]],
            ]
        ] = None,
        max_seq_len: int = 20,
        ...
        Args:
            ...
            loss: Callable that returns one or more (a list of) scalar loss
                terms.
            action_distribution_class: Class for a mindspore action distribution.
        ...
        gpu_ids = list(range(mindspore.cuda.device_count()))
        ...
        # - non-fake GPU mode.
            ...
            self.devices = [
                mindspore.device("cuda:{}".format(i))
    ...
    def compute_actions_from_input_dict(
        ...
        **kwargs,
        ...
        #     )
        # 设置上下文为eval模式，停止梯度计算
        original_context = context.get_context("device_target")
        ...
        try:
            ...
            # 恢复原来的上下文
            context.set_context(device_target=original_context)
    ...
    def compute_actions(
        ...
        **kwargs,
        ...
        #     )
        # 设置上下文为eval模式，停止梯度计算
        original_context = context.get_context("mode")
        context.set_context(mode=context.PYNATIVE_MODE)
        ...
        try:
            ...
            # 恢复原来的上下文
            context.set_context(mode=original_context)
    ...
    def optimizer(
        ...
        if hasattr(self, "config"):
            optimizers = [
                mindspore.nn.Adam(self.model.parameters(), lr=self.config["lr"])
            ]
        else:
            optimizers = [mindspore.nn.Adam(self.model.parameters())]
        if self.exploration:
        ...
        # w/o access to the original (custom) Model or Policy code).
            ...
            try:
                ...
                if os.path.exists(filename):
                    os.remove(filename)
                logger.warning(ERR_MSG_MindSpore_POLICY_CANNOT_SAVE_MODEL)
    ...
    def _multi_gpu_parallel_grad_calc(
        ...
        lock = threading.Lock()
        results = {}
        grad_enabled = mindspore.is_grad_enabled()

        def _worker(shard_idx, model, sample_batch, device):
            mindspore.set_grad_enabled(grad_enabled)
            try:
                with NullContextManager() if device.type == "cpu" else mindspore.cuda.device(  # noqa: E501
                    device
                ):
                    loss_out = force_list(

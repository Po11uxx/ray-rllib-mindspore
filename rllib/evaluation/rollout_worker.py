...
class RolloutWorker(ParallelIteratorWorker, EnvRunner):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if (
            ...
            elif self.config.framework_str == "mindspore":
                devices = [0]
    ...
    def setup_mindspore_data_parallel(
        self, url: str, world_rank: int, world_size: int, backend: str
    ) -> None:
        """Join a mindspore process group for distributed SGD."""

        logger.info(
            "Joining process group, url={}, world_rank={}, "
            "world_size={}, backend={}".format(url, world_rank, world_size, backend)
        )
        mindspore.communication.init(backend_name=backend)

        for pid, policy in self.policy_map.items():
            if not isinstance(policy, (MindSporePolicy, MindSporePolicyV2)):
                raise ValueError(
                    "This policy does not support MindSpore distributed", policy
                )
            policy.distributed_world_size = world_size
    ...
    def _build_policy_map(
        ...
        *,
        ...
        for name, policy_spec in sorted(policy_dict.items()):
            ...
            # print("1111111111111", new_policy)

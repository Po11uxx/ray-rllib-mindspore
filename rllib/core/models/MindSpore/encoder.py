...
from ray.rllib.core.models.MindSpore.base import MindSporeModel
from ray.rllib.core.models.MindSpore.primitives import MindSporeMLP, MindSporeCNN
...
class MindSporeActorCriticEncoder(MindSporeModel, ActorCriticEncoder):
    """An actor-critic encoder for mindspore."""

    framework = "mindspore"

    def __init__(self, config: ActorCriticEncoderConfig) -> None:
        MindSporeModel.__init__(self, config)
        ActorCriticEncoder.__init__(self, config)


class MindSporeStatefulActorCriticEncoder(MindSporeModel, StatefulActorCriticEncoder):
    """A stateful actor-critic encoder for mindspore."""
...
class MindSporeMLPEncoder(MindSporeModel, Encoder):
    def __init__(self, config: MLPEncoderConfig) -> None:
        MindSporeModel.__init__(self, config)
        Encoder.__init__(self, config)

        # Create the neural network.
        self.net = MindSporeMLP(
...
class MindSporeCNNEncoder(MindSporeModel, Encoder):
...
class MindSporeGRUEncoder(MindSporeModel, Encoder):
    ...
    def __init__(self, config: RecurrentEncoderConfig) -> None:
        ...
        if config.tokenizer_config is not None:
            self.tokenizer = config.tokenizer_config.build(framework="mindspore")
        ...
        gru_weights_initializer = get_initializer_fn(
            config.hidden_weights_initializer, framework="mindspore"
        )
        gru_bias_initializer = get_initializer_fn(
            config.hidden_bias_initializer, framework="mindspore"
        )

        # Create the mindspore GRU layer.
        self.gru = nn.GRU(
            gru_input_dim,
            config.hidden_dim,
            config.num_layers,
    ...
    def get_initial_state(self):
        return {
            "h": mindspore.ops.zeros(self.config.num_layers, self.config.hidden_dim),
        }
    ...
    def _forward(self, inputs: dict, **kwargs) -> dict:
        ...
        if self.tokenizer is not None:
            ...
            out = tokenize(self.tokenizer, inputs, framework="mindspore")
        else:
...
class MindSporeLSTMEncoder(MindSporeModel, Encoder):
    ...
    def __init__(self, config: RecurrentEncoderConfig) -> None:
        ...
        # Create the mindspore LSTM layer.
        self.lstm = nn.LSTM(
            lstm_input_dim,
            config.hidden_dim,
            config.num_layers,
    ...
    def get_initial_state(self):
        return {
            "h": mindspore.ops.zeros(self.config.num_layers, self.config.hidden_dim),
            "c": mindspore.ops.zeros(self.config.num_layers, self.config.hidden_dim),
        }

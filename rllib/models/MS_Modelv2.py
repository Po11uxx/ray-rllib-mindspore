...
from ray.rllib.utils.framework import try_import_tf, try_import_torch, TensorType, try_import_mindspore
...
class MS_ModelV2:
    ...
    def construct(
        ...
        __call__ before being passed to construct(). To access the flattened
        observation tensor, refer to input_dict["obs_flat"].

        This method can be called any number of times. In eager execution,
        each call to construct() will eagerly evaluate the model. In symbolic
        execution, each call to construct creates a computation graph that
        ...
        .. testcode::
            ...
            class MyModel(ModelV2):
                # ...
                def construct(self, input_dict, state, seq_lens):
    ...
    def value_function(self) -> TensorType:
        """Returns the value function output for the most recent construct pass.

        Note that a `construct` call has to be performed first, before this
        methods can return anything and thus that calling this method does not
        cause an extra construct pass through the network.
    ...
    def __call__(
        ...
        This is the method used by RLlib to execute the construct pass. It calls
        construct() internally after unpacking nested observation tensors.

        Custom models should override construct() instead of __call__.
        ...
        with self.context():
            res = self.construct(restored, state or [], seq_lens)
        ...
        if (not isinstance(res, list) and not isinstance(res, tuple)) or len(res) != 2:
            raise ValueError(
                "construct() must return a tuple of (output, state) tensors, "
                "got {}".format(res)
            )
    ...
    def context(self) -> contextlib.AbstractContextManager:
        """Returns a contextmanager for the current construct pass."""
...
def flatten(obs: TensorType, framework: str) -> TensorType:
    ...
    if framework in ["tf2", "tf"]:
        ...
        assert mindspore is not None
        return mindspore.flatten(obs, start_dim=1)
    else:

...
algorithm. See `dqn_[tf|torch|mindspore]_policy.py` for the definition of the policies.
...
class DQNConfig(AlgorithmConfig):
    ...
    def get_default_rl_module_spec(self) -> RLModuleSpecType:
        ...
        if self.framework_str == "torch":
            ...
            from ray.rllib.algorithms.dqn.MindSpore.dqn_rainbow_mindspore_rl_module import (
                DQNRainbowMindSporeRLModule,
            )

            return RLModuleSpec(
                module_class=DQNRainbowMindSporeRLModule,
                catalog_class=DQNRainbowCatalog,
                model_config=self.model_config,
            )
        else:
            raise ValueError(
                ...
                "Use `config.framework('torch') or 'mindspore'` instead."
            )
    ...
    def get_default_learner_class(self) -> Union[Type["Learner"], str]:
        if self.framework_str == "torch":
            ...
            return DQNRainbowMindSporeLearner
        else:
            raise ValueError(
                f"The framework {self.framework_str} is not supported! "
                "Use `config.framework('torch') or 'mindspore'` instead."

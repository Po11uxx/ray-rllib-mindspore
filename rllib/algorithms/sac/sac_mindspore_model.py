...
class SACMindSporeModel(MindSporeModelV2, nn.Cell):
    """Extension of the standard MindSporeModelV2 for SAC.

    To customize, do one of the following:
    - sub-class SACMindSporeModel and override one or more of its methods.
    ...
    def __init__(
        ...
        """Initializes a SACMindSporeModel instance.
        7
                Args:
        ...
        super(SACMindSporeModel, self).__init__(
        ...
        log_alpha = nn.Parameter(
            mindspore.from_numpy(np.array([np.log(initial_alpha)])).float()
        )
        ...
        target_entropy = nn.Parameter(
            mindspore.from_numpy(np.array([target_entropy])).float(), requires_grad=False
        )
    ...
    def build_policy_model(self, obs_space, num_outputs, policy_model_config, name):
        ...
        Returns:
            MindSporeModelV2: The MindSporeModelV2 policy sub-model.
        """
    ...
    def build_q_model(self, obs_space, action_space, num_outputs, q_model_config, name):
        ...
        Returns:
            MindSporeModelV2: The MindSporeModelV2 Q-net sub-model.
        """
    ...
    def _get_q_value(self, model_out, actions, net):
        ...
        if isinstance(net.obs_space, Box):
            if isinstance(model_out, (list, tuple)):
                model_out = mindspore.ops.cat(model_out, dim=-1)
            elif isinstance(model_out, dict):
                model_out = mindspore.ops.cat(list(model_out.values()), dim=-1)

        # Continuous case -> concat actions to model_out.
        if actions is not None:
            if self.concat_obs_and_actions:
                input_dict = {"obs": mindspore.ops.cat([model_out, actions], dim=-1)}
            else:
    ...
    def get_action_model_outputs(
        ...
        def concat_obs_if_necessary(obs: TensorStructType):
            ...
            if isinstance(obs, (list, tuple)):
                obs = mindspore.ops.cat(obs, dim=-1)
            elif isinstance(obs, dict):
                obs = mindspore.ops.cat(
                    [
                        mindspore.ops.unsqueeze(val, 1) if len(val.shape) == 1 else val

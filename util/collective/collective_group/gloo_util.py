...
from ray.util.collective.types import ReduceOp, torch_available, mindspore_available
...
elif mindspore_available():
    import mindspore

    MINDSPORE_GLOO_DTYPE_MAP = {
        mindspore.int: pygloo.glooDataType_t.glooInt32,
        mindspore.uint8: pygloo.glooDataType_t.glooUint8,
        mindspore.int8: pygloo.glooDataType_t.glooInt8,
        mindspore.int32: pygloo.glooDataType_t.glooInt32,
        mindspore.int64: pygloo.glooDataType_t.glooInt64,
        mindspore.long: pygloo.glooDataType_t.glooInt64,
        # FLOAT types
        mindspore.half: pygloo.glooDataType_t.glooFloat16,
        mindspore.float: pygloo.glooDataType_t.glooFloat32,
        mindspore.float16: pygloo.glooDataType_t.glooFloat16,
        mindspore.float32: pygloo.glooDataType_t.glooFloat32,
        mindspore.float64: pygloo.glooDataType_t.glooFloat64,
        mindspore.double: pygloo.glooDataType_t.glooFloat64,
    }

    MINDSPORE_NUMPY_DTYPE_MAP = {
        # INT types
        mindspore.int: numpy.int32,
        mindspore.uint8: numpy.uint8,
        mindspore.int8: numpy.int8,
        mindspore.int32: numpy.int32,
        mindspore.int64: numpy.int64,
        mindspore.long: numpy.int64,
        # FLOAT types
        mindspore.half: numpy.half,
        mindspore.float: numpy.float32,
...
def get_gloo_tensor_dtype(tensor):
    ...
    if torch_available():
        ...
        if isinstance(tensor, mindspore.Tensor):
            if not tensor.is_cuda:
                return MINDSPORE_GLOO_DTYPE_MAP[tensor.dtype]
            else:
                raise ValueError(
                    "Expect mindspore CPU tensor. Got {}.".format(tensor.device)
                )
...
def get_numpy_tensor_dtype(tensor):
    ...
    if torch_available():
        ...
        if isinstance(tensor, mindspore.Tensor):
            return MINDSPORE_NUMPY_DTYPE_MAP[tensor.dtype]
    raise ValueError(
        "Unsupported tensor type. Got: {}. Supported "
        "CPU tensor types are: mindspore.Tensor, "
...
def get_tensor_ptr(tensor):
    ...
    if torch_available():
        ...
        if isinstance(tensor, mindspore.Tensor):
            if tensor.is_cuda:
                raise RuntimeError(
                    "mindspore tensor must be on CPU when using GLOO collectives."
                )
            return tensor.data_ptr()
    raise ValueError(
        "Unsupported tensor type. Got: {}. Supported "
...
def get_tensor_device(tensor):
    if isinstance(tensor, numpy.ndarray):
        ...
        if not tensor.is_cuda:
            ...
            return "cuda"
    elif mindspore_available() and isinstance(tensor, mindspore.Tensor):
        if not tensor.is_cuda:
            return "cpu"
        else:
            return "cuda"
...
def copy_tensor(dst_tensor, src_tensor):
    ...
    if isinstance(dst_tensor, numpy.ndarray) and isinstance(src_tensor, numpy.ndarray):
        ...
        if isinstance(dst_tensor, mindspore.Tensor) and isinstance(
            src_tensor, mindspore.Tensor
        ):
            dst_tensor.copy_(src_tensor)
        elif isinstance(dst_tensor, mindspore.Tensor) and isinstance(
            src_tensor, numpy.ndarray
        ):
            t = mindspore.Tensor(src_tensor)
            dst_tensor.copy_(t)
        elif isinstance(dst_tensor, numpy.ndarray) and isinstance(
            src_tensor, mindspore.Tensor
        ):

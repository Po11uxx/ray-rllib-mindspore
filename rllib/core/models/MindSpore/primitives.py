...
from ray.rllib.core.models.MindSpore.utils import Stride2D
from ray.rllib.models.MindSpore.misc import (
    same_padding,
...
class MindSporeMLP(nn.Cell):
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize a MindSporeMLP object.
        ...
        Args:
            ...
            hidden_layer_activation: The activation function to use after each layer
                (except for the output). Either a mindspore.nn.[activation fn] callable or
            ...
            output_activation: The activation function to use for the output layer
                (if any). Either a mindspore.nn.[activation fn] callable or
                the name thereof, or an RLlib recognized activation name,
                e.g. "ReLU", "relu", "tanh", "SiLU", or "linear".
        ...
        hidden_activation = get_activation_fn(
            hidden_layer_activation, framework="mindspore"
        )
        hidden_weights_initializer = get_initializer_fn(
            hidden_layer_weights_initializer, framework="mindspore"
        )
        hidden_bias_initializer = get_initializer_fn(
            hidden_layer_bias_initializer, framework="mindspore"
        )
        output_weights_initializer = get_initializer_fn(
            output_weights_initializer, framework="mindspore"
        )
        output_bias_initializer = get_initializer_fn(
            output_bias_initializer, framework="mindspore"
        )
        ...
        for i in range(0, len(dims) - 1):
            ...
            layer = mindspore.mint.nn.Linear(
                dims[i],
                dims[i + 1],
        ...
        output_activation = get_activation_fn(output_activation, framework="mindspore")
...
class MindSporeCNN(nn.Cell):
    ...
    def __init__(
        ...
        *,
        ...
        """Initializes a MindSporeCNN instance.
        ...
        cnn_activation = get_activation_fn(cnn_activation, framework="mindspore")
        cnn_kernel_initializer = get_initializer_fn(
            cnn_kernel_initializer, framework="mindspore"
        )
        cnn_bias_initializer = get_initializer_fn(
            cnn_bias_initializer, framework="mindspore"
        )
        layers = []
    ...
    def forward(self, inputs):
        ...
        # Convert to `channels_first` for mindspore:
...
class MindSporeCNNTranspose(nn.Cell):
    ...
    def __init__(
        ...
        *,
        ...
        """Initializes a MindSporeCNNTranspose instance.
        ...
        cnn_transpose_activation = get_activation_fn(
            cnn_transpose_activation, framework="mindspore"
        )
        cnn_transpose_kernel_initializer = get_initializer_fn(
            cnn_transpose_kernel_initializer, framework="mindspore"
        )
        cnn_transpose_bias_initializer = get_initializer_fn(
            cnn_transpose_bias_initializer, framework="mindspore"
        )
        ...
        for i, (out_depth, kernel, stride) in enumerate(
            ...
            layer = nn.ConvTranspose2d(
                ...
                1,
                # Disable mindspore auto-padding (mindspore interprets the padding setting

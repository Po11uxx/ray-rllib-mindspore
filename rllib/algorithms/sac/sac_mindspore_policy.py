"""
MindSpore policy class used for SAC.
"""
...
from ray.rllib.models.MindSpore.mindspore_action_dist import (
    MindSporeCategorical,
    MindSporeDistributionWrapper,
    MindSporeDirichlet,
...
from ray.rllib.policy.mindspore_mixins import TargetNetworkMixin
...
def _get_dist_class(
...
) -> Type[MindSporeDistributionWrapper]:
    ...
    if hasattr(policy, "dist_class") and policy.dist_class is not None:
        ...
        action_dist_class, _ = ModelCatalog.get_action_dist(
            action_space, config["model"], framework="mindspore"
        )
        return action_dist_class
    elif isinstance(action_space, Discrete):
        return MindSporeCategorical
    elif isinstance(action_space, Simplex):
        return MindSporeDirichlet
    else:
...
) -> Tuple[ModelV2, Type[MindSporeDistributionWrapper]]:
    ...
    *,
    ...
    Args:
        policy: The Policy being queried for actions and calling this
            function.
        model (MindSporeModelV2): The SAC specific model to use to generate the
            distribution inputs (see sac_tf|mindspore_model.py). Must support the
...
def actor_critic_loss(
    ...
    Args:
        ...
        dist_class (Type[MindSporeDistributionWrapper]: The action distr. class.
    ...
    alpha = mindspore.ops.exp(model.log_alpha)
    ...
    if model.discrete:
        ...
        if policy.config["twin_q"]:
            ...
            q_tp1 = mindspore.ops.min(q_tp1, twin_q_tp1)
        ...
        q_t_selected = mindspore.ops.sum(q_t * one_hot, dim=-1)
        if policy.config["twin_q"]:
            twin_q_t_selected = mindspore.ops.sum(twin_q_t * one_hot, dim=-1)
        # Discrete case: "Best" means weighted by the policy (prob) outputs.
        q_tp1_best = mindspore.ops.sum(mindspore.mul(policy_tp1, q_tp1), dim=-1)
        q_tp1_best_masked = (
    ...
    # Continuous actions case.
        ...
        log_pis_t = mindspore.ops.unsqueeze(action_dist_t.logp(policy_t), -1)
        action_dist_inputs_tp1, _ = model.get_action_model_outputs(model_out_tp1)
        ...
        log_pis_tp1 = mindspore.ops.unsqueeze(action_dist_tp1.logp(policy_tp1), -1)
        ...
        if policy.config["twin_q"]:
            ...
            q_t_det_policy = mindspore.ops.min(q_t_det_policy, twin_q_t_det_policy)
        ...
        q_t_selected = mindspore.ops.squeeze(q_t, dim=-1)
        if policy.config["twin_q"]:
            twin_q_t_selected = mindspore.ops.squeeze(twin_q_t, dim=-1)
        q_tp1 -= alpha * log_pis_tp1

        q_tp1_best = mindspore.ops.squeeze(input=q_tp1, dim=-1)
        q_tp1_best_masked = (
            1.0 - train_batch[SampleBatch.TERMINATEDS].float()
        ) * q_tp1_best
    ...
    base_td_error = mindspore.ops.abs(q_t_selected - q_t_selected_target)
    if policy.config["twin_q"]:
        twin_td_error = mindspore.ops.abs(twin_q_t_selected - q_t_selected_target)
        td_error = 0.5 * (base_td_error + twin_td_error)
    else:
        td_error = base_td_error

    critic_loss = [mindspore.ops.mean(train_batch[PRIO_WEIGHTS] * huber_loss(base_td_error))]
    if policy.config["twin_q"]:
        critic_loss.append(
            mindspore.ops.mean(train_batch[PRIO_WEIGHTS] * huber_loss(twin_td_error))
        )
    ...
    if model.discrete:
        ...
        alpha_loss = mindspore.ops.mean(mindspore.ops.sum(weighted_log_alpha_loss, dim=-1))
        # Actor loss.
        actor_loss = mindspore.ops.mean(
            mindspore.ops.sum(
                mindspore.ops.mul(
        ...
        actor_loss = mindspore.ops.mean(alpha.detach() * log_pis_t - q_t_det_policy)
...
def stats(policy: Policy, train_batch: SampleBatch) -> Dict[str, TensorType]:
    ...
    """
    q_t = mindspore.ops.stack(policy.get_tower_stats("q_t"))

    return {
        "actor_loss": mindspore.ops.mean(mindspore.ops.stack(policy.get_tower_stats("actor_loss"))),
        "critic_loss": mindspore.ops.mean(
            mindspore.ops.stack(tree.flatten(policy.get_tower_stats("critic_loss")))
        ),
        "alpha_loss": mindspore.ops.mean(mindspore.ops.stack(policy.get_tower_stats("alpha_loss"))),
        "alpha_value": mindspore.ops.exp(policy.model.log_alpha),
        "log_alpha_value": policy.model.log_alpha,
        "target_entropy": policy.model.target_entropy,
        "policy_t": mindspore.ops.mean(mindspore.ops.stack(policy.get_tower_stats("policy_t"))),
        "mean_q": mindspore.ops.mean(q_t),
        "max_q": mindspore.ops.max(q_t),
        "min_q": mindspore.ops.min(q_t),
    }
...
def optimizer_fn(policy: Policy, config: AlgorithmConfigDict) -> Tuple[LocalOptimizer]:
    ...
    """
    policy.actor_optim = mindspore.nn.Adam(
    ...
    policy.alpha_optim = mindspore.nn.Adam(
...
# Build a child class of `MindSporePolicy`, given the custom functions defined
# above.
SACMindSporePolicy = build_ms_policy_class(
    name="SACMindSporePolicy",
    framework="mindspore",

...
@OldAPIStack
class MindSporeDistributionWrapper(ActionDistribution):
    """Wrapper class for mindspore.distributions."""

    @override(ActionDistribution)
    def __init__(self, inputs: List[TensorType], model: MindSporeModelV2):
        # If inputs are not a mindspore Tensor, make them one and make sure they
        # are on the correct device.
        if not isinstance(inputs, mindspore.Tensor):
            inputs = mindspore.Tensor.from_numpy(inputs)
            if isinstance(model, MindSporeModelV2):
    ...
    def kl(self, other: ActionDistribution) -> TensorType:
        # return mindspore.ops.kl_div(self.dist, other.dist)
        logits1 = self.logits

        # 获取第二个分布的 logits
        logits2 = other.logits

        # 计算 KL 散度
        # KL散度的计算公式是：KL(p || q) = sum(p * (log(p) - log(q)))
        # 其中 p 和 q 是两个分布的概率值
        probs1 = mindspore.ops.softmax(logits1, axis=-1)
        probs2 = mindspore.ops.softmax(logits2, axis=-1)
        kl_div_value = mindspore.ops.reduce_sum(probs1 * (mindspore.ops.log(probs1) - mindspore.ops.log(probs2)), axis=-1)
...
@OldAPIStack
class MindSporeCategorical(MindSporeDistributionWrapper):
    """Wrapper class for MindSpore Categorical distribution."""

    @override(ActionDistribution)
    def __init__(
        self,
        inputs: List[TensorType],
        model: MindSporeModelV2 = None,
        ...
        self.dist = mindspore.nn.probability.distribution.Categorical(probs=probs, dtype=mindspore.int32)

    @override(ActionDistribution)
    def deterministic_sample(self) -> TensorType:
        self.last_sample = self.dist.probs.argmax()
        return self.last_sample
...
@OldAPIStack
def get_mindspore_categorical_class_with_temperature(t: float):
    """MindSporeCategorical distribution class that has customized default temperature."""

    class MindSporeCategoricalWithTemperature(MindSporeCategorical):
        def __init__(self, inputs, model=None, temperature=t):
            super().__init__(inputs, model, temperature)

    return MindSporeCategoricalWithTemperature


@OldAPIStack
class MindSporeMultiCategorical(MindSporeDistributionWrapper):
    """MultiCategorical distribution for MultiDiscrete action spaces."""

    @override(MindSporeDistributionWrapper)
    def __init__(
        self,
        inputs: List[TensorType],
        ...
        self.cats = [
            mindspore.nn.probability.distribution.Categorical(logits=input_)
    ...
    def sample(self) -> TensorType:
        ...
        if isinstance(self.action_space, gym.spaces.Box):
            sample_ = mindspore.ops.reshape(sample_, [-1] + list(self.action_space.shape))
        self.last_sample = sample_
        return sample_

    @override(ActionDistribution)
    def deterministic_sample(self) -> TensorType:
        arr = [mindspore.argmax(cat.probs, -1) for cat in self.cats]
        sample_ = mindspore.ops.stack(arr, dim=1)
        if isinstance(self.action_space, gym.spaces.Box):
            sample_ = mindspore.ops.reshape(sample_, [-1] + list(self.action_space.shape))
        self.last_sample = sample_
    ...
    def logp(self, actions: TensorType) -> TensorType:
        ...
        if isinstance(actions, mindspore.Tensor):
            if isinstance(self.action_space, gym.spaces.Box):
                actions = mindspore.ops.reshape(
                    actions, [-1, int(np.prod(self.action_space.shape))]
                )
            actions = mindspore.ops.unbind(actions, dim=1)
        logps = mindspore.ops.stack([cat.log_prob(act) for cat, act in zip(self.cats, actions)])
        return mindspore.ops.sum(logps, dim=0)

    @override(ActionDistribution)
    def multi_entropy(self) -> TensorType:
        return mindspore.ops.stack([cat.entropy() for cat in self.cats], dim=1)

    @override(MindSporeDistributionWrapper)
    def entropy(self) -> TensorType:
        return mindspore.ops.sum(self.multi_entropy(), dim=1)

    @override(ActionDistribution)
    def multi_kl(self, other: ActionDistribution) -> TensorType:
        return mindspore.ops.stack(
            [
                mindspore.ops.kl_div(cat, oth_cat)
    ...
    @override(MindSporeDistributionWrapper)
    def kl(self, other: ActionDistribution) -> TensorType:
        return mindspore.ops.sum(self.multi_kl(other), dim=1)
...
@OldAPIStack
class MindSporeSlateMultiCategorical(MindSporeCategorical):
    """MultiCategorical distribution for MultiDiscrete action spaces.
    ...
    def deterministic_sample(self) -> TensorType:
        ...
        return mindspore.take_along_dim(self.all_slates, sample.long(), dim=-1)

    @override(ActionDistribution)
    def logp(self, x: TensorType) -> TensorType:
        # TODO: Implement.
        return mindspore.ops.ones_like(self.inputs[:, 0])


@OldAPIStack
class MindSporeDiagGaussian(MindSporeDistributionWrapper):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(inputs, model)
        mean, log_std = mindspore.ops.chunk(self.inputs, 2, dim=1)
        self.log_std = log_std
        self.dist = mindspore.nn.probability.distribution.Normal(mean, mindspore.ops.exp(log_std))
    ...
    def sample(self) -> TensorType:
        ...
        if self.zero_action_dim:
            return mindspore.ops.squeeze(sample, dim=-1)
        return sample
...
@OldAPIStack
class MindSporeSquashedGaussian(MindSporeDistributionWrapper):
    ...
    def __init__(
        ...
        std = mindspore.ops.exp(log_std)
        self.dist = mindspore.nn.probability.distribution.Normal(mean, std)
    ...
    def logp(self, x: TensorType) -> TensorType:
        ...
        log_prob_gaussian = mindspore.ops.clamp(log_prob_gaussian, -100, 100)
        log_prob_gaussian = mindspore.ops.sum(log_prob_gaussian, dim=-1)
        # Get log-prob for squashed Gaussian.
        unsquashed_values_tanhd = mindspore.ops.tanh(unsquashed_values)
        log_prob = log_prob_gaussian - mindspore.ops.sum(
            mindspore.ops.log(1 - unsquashed_values_tanhd**2 + SMALL_NUMBER), dim=-1
        )
        return log_prob
    ...
    def sample_logp(self):
        ...
        return actions, mindspore.ops.sum(
            self.dist.log_prob(z) - mindspore.ops.log(1 - actions * actions + SMALL_NUMBER),
            dim=-1,
        )
    ...
    def _squash(self, raw_values: TensorType) -> TensorType:
        ...
        squashed = ((mindspore.ops.tanh(raw_values) + 1.0) / 2.0) * (
            self.high - self.low
        ) + self.low
        return mindspore.ops.clamp(squashed, self.low, self.high)

    def _unsquash(self, values: TensorType) -> TensorType:
        normed_values = (values - self.low) / (self.high - self.low) * 2.0 - 1.0
        # Stabilize input to atanh.
        save_normed_values = mindspore.ops.clamp(
            normed_values, -1.0 + SMALL_NUMBER, 1.0 - SMALL_NUMBER
        )
        unsquashed = mindspore.ops.atanh(save_normed_values)
        return unsquashed
...
@OldAPIStack
class MindSporeBeta(MindSporeDistributionWrapper):
    """
    ...
    def __init__(
        ...
        self.inputs = mindspore.ops.clamp(self.inputs, log(SMALL_NUMBER), -log(SMALL_NUMBER))
        self.inputs = mindspore.ops.log(mindspore.exp(self.inputs) + 1.0) + 1.0
        self.low = low
        self.high = high
        alpha, beta = mindspore.ops.chunk(self.inputs, 2, dim=-1)
        # Note: concentration0==beta, concentration1=alpha (!)
        self.dist = mindspore.nn.probability.distribution.Beta(concentration1=alpha, concentration0=beta)
    ...
    def logp(self, x: TensorType) -> TensorType:
        unsquashed_values = self._unsquash(x)
        return mindspore.ops.sum(self.dist.log_prob(unsquashed_values), dim=-1)
...
@OldAPIStack
class MindSporeDeterministic(MindSporeDistributionWrapper):
    ...
    @override(MindSporeDistributionWrapper)
    def sampled_action_logp(self) -> TensorType:
        return mindspore.ops.zeros((self.inputs.size()[0],), dtype=float32)
...
@OldAPIStack
class MindSporeMultiActionDistribution(MindSporeDistributionWrapper):
    ...
    def __init__(self, inputs, model, *, child_distributions, input_lens, action_space):
        ...
        Args:
            inputs (mindspore.Tensor): A single tensor of shape [BATCH, size].
            model (MindSporeModelV2): The MindSporeModelV2 object used to produce
                inputs for this distribution.
            child_distributions (any[mindspore.Tensor]): Any struct
        ...
        split_inputs = mindspore.ops.split(inputs, self.input_lens, dim=1)
    ...
    def logp(self, x):
        ...
        if isinstance(x, mindspore.Tensor):
            ...
            for dist in self.flat_child_distributions:
                if isinstance(dist, MindSporeCategorical):
                    ...
                    isinstance(dist, MindSporeMultiCategorical)
            ...
            split_x = list(mindspore.ops.split(x, split_indices, dim=1))
        ...
        def map_(val, dist):
            ...
            if isinstance(dist, MindSporeCategorical):
                val = (mindspore.ops.squeeze(val, dim=-1) if len(val.shape) > 1 else val).int()
...
@OldAPIStack
class MindSporeDirichlet(MindSporeDistributionWrapper):
    ...
    def __init__(self, inputs, model):
        ...
        """
        self.epsilon = mindspore.tensor(1e-7).to(inputs.device)
        concentration = mindspore.ops.exp(inputs) + self.epsilon
        # TODO: to be determined
        self.dist = mindspore.distributions.dirichlet.Dirichlet(
    ...
    @override(ActionDistribution)
    def deterministic_sample(self) -> TensorType:
        self.last_sample = mindspore.ops.softmax(self.dist.concentration, dim=-1)
        return self.last_sample
    ...
    def logp(self, x):
        ...
        # numerical errors.
        x = mindspore.ops.max(x, self.epsilon)
        x = x / mindspore.ops.sum(x, dim=-1, keepdim=True)

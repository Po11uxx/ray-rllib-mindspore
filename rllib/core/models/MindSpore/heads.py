...
from ray.rllib.core.models.MindSpore.primitives import MindSporeCNNTranspose, MindSporeMLP
...
class MindSporeMLPHead(MindSporeModel):
...
class MindSporeFreeLogStdMLPHead(MindSporeModel):
    ...
    def __init__(self, config: FreeLogStdMLPHeadConfig) -> None:
        ...
        self.log_std = mindspore.nn.Parameter(
            mindspore.as_tensor([0.0] * self._half_output_dim)
        )
        # If log standard deviations should be clipped. This should be only true for
        # policy heads. Value heads should never be clipped.
        self.clip_log_std = config.clip_log_std
        # The clipping parameter for the log standard deviation.
        self.log_std_clip_param = mindspore.Tensor(
    ...
    def _forward(self, inputs: mindspore.Tensor, **kwargs) -> mindspore.Tensor:
        ...
        return mindspore.ops.cat([mean, log_std.unsqueeze(0).repeat([len(mean), 1])], axis=1)


class MindSporeCNNTransposeHead(MindSporeModel):
    def __init__(self, config: CNNTransposeHeadConfig) -> None:
        ...
        initial_dense_weights_initializer = get_initializer_fn(
            config.initial_dense_weights_initializer, framework="mindspore"
        )
        initial_dense_bias_initializer = get_initializer_fn(
            config.initial_dense_bias_initializer, framework="mindspore"
        )
        ...
        self.cnn_transpose_net = MindSporeCNNTranspose(

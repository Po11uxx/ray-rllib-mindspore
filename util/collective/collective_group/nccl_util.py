...
if torch_available():
    ...
    import mindspore
    import mindspore.utils.dlpack

    MINDSPORE_NCCL_DTYPE_MAP = {
        mindspore.bool: nccl.NCCL_INT8,
        # INT types
        mindspore.int: nccl.NCCL_INT,
        mindspore.uint8: nccl.NCCL_UINT8,
        mindspore.int8: nccl.NCCL_INT8,
        mindspore.int32: nccl.NCCL_INT32,
        mindspore.int64: nccl.NCCL_INT64,
        mindspore.long: nccl.NCCL_INT64,
        # FLOAT types
        mindspore.half: nccl.NCCL_HALF,
        mindspore.float: nccl.NCCL_FLOAT,
        mindspore.float16: nccl.NCCL_FLOAT16,
        mindspore.float32: nccl.NCCL_FLOAT32,
        mindspore.float64: nccl.NCCL_FLOAT64,
        mindspore.double: nccl.NCCL_DOUBLE,
    }

    # Older versions of cupy don't support bfloat16.
    if hasattr(nccl, "NCCL_BFlOAT16"):
        MINDSPORE_NCCL_DTYPE_MAP[mindspore.bfloat16] = nccl.NCCL_BFLOAT16
...
def get_nccl_tensor_dtype(tensor):
    ...
    if torch_available():
        ...
        if isinstance(tensor, mindspore.Tensor):
            return MINDSPORE_NCCL_DTYPE_MAP[tensor.dtype]
    raise ValueError(
...
def get_tensor_ptr(tensor):
    ...
    if torch_available():
        ...
        if isinstance(tensor, mindspore.Tensor):
            if not tensor.is_cuda:
                raise RuntimeError(
                    "mindspore tensor must be on GPU when using NCCL collectives."
                )
            return tensor.data_ptr()
    raise ValueError(
        "Unsupported tensor type. Got: {}. Supported "
...
def copy_tensor(dst_tensor, src_tensor):
    ...
    if isinstance(dst_tensor, cupy.ndarray) and isinstance(src_tensor, cupy.ndarray):
        ...
        if isinstance(dst_tensor, mindspore.Tensor) and isinstance(
            ...
            t = mindspore.utils.dlpack.from_dlpack(src_tensor.toDlpack())
            dst_tensor.copy_(t)
        elif isinstance(dst_tensor, cupy.ndarray) and isinstance(
            src_tensor, mindspore.Tensor
        ):
            t = cupy.fromDlpack(mindspore.utils.dlpack.to_dlpack(src_tensor))
            cupy.copyto(dst_tensor, t)
        else:
            copied = False
    else:

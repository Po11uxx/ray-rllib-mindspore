...
from ray.rllib.utils.framework import try_import_jax, try_import_tf, try_import_torch, try_import_mindspore
...
def get_initializer_fn(name: Optional[Union[str, Callable]], framework: str = "torch"):
    ...
    if framework == "torch":
        ...
        # Check, if the name includes an underscore. We must use the
        # in-place initialization from mindspore.
        if not name_lower.endswith("_"):
            raise ValueError(
                "Not an in-place initializer: mindspore weight initializers "
                "need to be provided as their in-place version, i.e. "
                "<initializaer_name> + '_'. See "
                "https://www.mindspore.cn/docs/zh-CN/r2.3.1/api_python/mindspore.nn.html ."
                f"User provided {name}."
            )
...
def get_initializer(name, framework="tf"):
    ...
    if framework == "torch":
        ...
        _, nn = try_import_mindspore()
        assert nn is not None, "`mindspore` not installed. Try `pip install mindspore`."
        if name in [None, "default", "xavier_uniform"]:
            return nn.init.xavier_uniform_
        elif name == "xavier_normal":
            return nn.init.xavier_normal_

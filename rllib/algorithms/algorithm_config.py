...
from ray.rllib.utils.framework import try_import_tf, try_import_torch, try_import_mindspore
...
from ray.rllib.utils.mindspore_utils import MINDSPORE_COMPILE_REQUIRED_VERSION
...
class AlgorithmConfig(_Config):
    ...
    def __init__(self, algo_class: Optional[type] = None):
        ...
        # MindSpore compile settings
        self.mindspore_compile_learner = False
        self.mindspore_compile_learner_what_to_compile = (
            MindSporeCompileWhatToCompile.FORWARD_TRAIN
        )
        ...
        # AOT Eager is a dummy backend and doesn't result in speedups.
        self.mindspore_compile_learner_dynamo_backend = (
            "aot_eager" if sys.platform == "darwin" else "inductor"
        )
        self.mindspore_compile_learner_dynamo_mode = None
        self.mindspore_compile_worker = False
        # AOT Eager is a dummy backend and doesn't result in speedups.
        self.mindspore_compile_worker_dynamo_backend = (
            "aot_eager" if sys.platform == "darwin" else "onnxrt"
        )
        self.mindspore_compile_worker_dynamo_mode = None
        # Default kwargs for `mindspore.nn.parallel.DistributedDataParallel`.
        self.mindspore_ddp_kwargs = {}
        # Default setting for skipping `nan` gradient updates.
        self.mindspore_skip_nan_gradients = False
        ...
        self._mindspore_grad_scaler_class = None
        self._mindspore_lr_scheduler_classes = None
    ...
    def framework(
        ...
        *,
        ...
        mindspore_compile_learner: Optional[bool] = NotProvided,
        mindspore_compile_learner_what_to_compile: Optional[str] = NotProvided,
        mindspore_compile_learner_dynamo_mode: Optional[str] = NotProvided,
        mindspore_compile_learner_dynamo_backend: Optional[str] = NotProvided,
        mindspore_compile_worker: Optional[bool] = NotProvided,
        mindspore_compile_worker_dynamo_backend: Optional[str] = NotProvided,
        mindspore_compile_worker_dynamo_mode: Optional[str] = NotProvided,
        mindspore_ddp_kwargs: Optional[Dict[str, Any]] = NotProvided,
        mindspore_skip_nan_gradients: Optional[bool] = NotProvided,
    ) -> "AlgorithmConfig":
        ...
        if mindspore_compile_learner is not NotProvided:
            self.mindspore_compile_learner = mindspore_compile_learner
        if mindspore_compile_learner_dynamo_backend is not NotProvided:
            self.mindspore_compile_learner_dynamo_backend = (
                mindspore_compile_learner_dynamo_backend
            )
        if mindspore_compile_learner_dynamo_mode is not NotProvided:
            self.mindspore_compile_learner_dynamo_mode = mindspore_compile_learner_dynamo_mode
        if mindspore_compile_learner_what_to_compile is not NotProvided:
            self.mindspore_compile_learner_what_to_compile = (
                mindspore_compile_learner_what_to_compile
            )
        if mindspore_compile_worker is not NotProvided:
            self.mindspore_compile_worker = mindspore_compile_worker
        if mindspore_compile_worker_dynamo_backend is not NotProvided:
            self.mindspore_compile_worker_dynamo_backend = (
                mindspore_compile_worker_dynamo_backend
            )
        if mindspore_compile_worker_dynamo_mode is not NotProvided:
            self.mindspore_compile_worker_dynamo_mode = mindspore_compile_worker_dynamo_mode
        if mindspore_ddp_kwargs is not NotProvided:
            self.mindspore_ddp_kwargs = mindspore_ddp_kwargs
        if mindspore_skip_nan_gradients is not NotProvided:
            self.mindspore_skip_nan_gradients = mindspore_skip_nan_gradients
    ...
    def env_runners(
        ...
        *,
        ...
        print("env_runner_cls:", env_runner_cls)
    ...
    def experimental(
        ...
        *,
        ...
        _mindspore_grad_scaler_class: Optional[Type] = NotProvided,
        _mindspore_lr_scheduler_classes: Optional[
            Union[List[Type], Dict[ModuleID, List[Type]]]
        ] = NotProvided,
        ...
        if _mindspore_grad_scaler_class is not NotProvided:
            self._mindspore_grad_scaler_class = _mindspore_grad_scaler_class
        if _mindspore_lr_scheduler_classes is not NotProvided:
            self._mindspore_lr_scheduler_classes = _mindspore_lr_scheduler_classes
    ...
    def get_mindspore_compile_worker_config(self):
        """Returns the mindsporeCompileConfig to use on workers."""

        from ray.rllib.core.rl_module.mindspore.mindspore_compile_config import (
            MindSporeCompileConfig,
        )

        return MindSporeCompileConfig(
            mindspore_dynamo_backend=self.mindspore_compile_worker_dynamo_backend,
            mindspore_dynamo_mode=self.mindspore_compile_worker_dynamo_mode,
        )
    ...
    def _validate_framework_settings(self) -> None:
        ...
        _torch = None
        _mindspore = None
        if self.framework_str not in {"tf", "tf2"} and self.framework_str != "torch" and self.framework_str != "mindspore":
            return
        elif self.framework_str in {"tf", "tf2"}:
            _tf1, _tf, _tfv = try_import_tf()
        elif self.framework_str == "mindspore":
            _mindspore, _ = try_import_mindspore()
        else:
        ...
        if (
            _mindspore is not None
            and self.framework_str == "mindspore"
            and version.parse(_mindspore.__version__) < MINDSPORE_COMPILE_REQUIRED_VERSION
            and (self.mindspore_compile_learner or self.mindspore_compile_worker)
        ):
        ...
        if self.mindspore_compile_learner:
            from ray.rllib.core.learner.MindSpore.mindspore_learner import (
                MindSporeCompileWhatToCompile,
            )

            if self.mindspore_compile_learner_what_to_compile not in [
                MindSporeCompileWhatToCompile.FORWARD_TRAIN,
                MindSporeCompileWhatToCompile.COMPLETE_UPDATE,
            ]:
                raise ValueError(
                    f"`config.mindspore_compile_learner_what_to_compile` must be one of ["
                    f"MindSporeCompileWhatToCompile.forward_train, "
                    f"MindSporeCompileWhatToCompile.complete_update] but is"
                    f" {self.mindspore_compile_learner_what_to_compile}"
                )

        self._check_if_correct_nn_framework_installed(_tf1, _tf, _torch, _mindspore)
    ...
    def _validate_to_be_deprecated_settings(self):
        ...
        # plan).
            ...
            if self.framework_str not in ["tf", "torch", "mindspore"]:
            ...
            # if all policies used are DynamicTFPolicies or TorchPolicies).
                ...
                if any(
                    ...
                    or not issubclass(
                        spec.policy_class or default_policy_cls,
                        (DynamicTFPolicy, MindSporePolicy),
                    )
    ...
    def _check_if_correct_nn_framework_installed(self, _tf1, _tf, _torch, _mindspore):
        ...
        if self.framework_str in {"tf", "tf2"}:
            ...
            if not _mindspore:
                raise ImportError(
                    (
                        "Pymindspore was specified as the framework to use (via `config."
                        "framework('mindspore')`)! However, no installation was found. You "
                        "can install Pymindspore via `pip install mindspore`."
                    )
                )
...
class MindSporeCompileWhatToCompile(str, Enum):
    """Enumerates schemes of what parts of the TorchLearner can be compiled.

...
from ray.rllib.models.MindSpore.misc import SlimFC as MindSporeSlimFC
...
from ray.rllib.utils.mindspore_utils import one_hot as ms_one_hot
...
class Curiosity(Exploration):
    ...
    def get_exploration_optimizer(self, optimizers):
        ...
        if self.framework == "torch":
            ...
            self._optimizer = mindspore.nn.Adam(
                forward_params + inverse_params + feature_params, lr=self.lr
            )
        else:
    ...
    def postprocess_trajectory(self, policy, sample_batch, tf_sess=None):
        ...
        if self.framework == "torch":
            ...
            self._postprocess_mindspore(policy, sample_batch)
        else:
    ...
    def _postprocess_mindspore(self, policy, sample_batch):
        # Push both observations through feature net to get both phis.
        phis, _ = self.model._curiosity_feature_net(
            {
                SampleBatch.OBS: mindspore.ops.cat(
                    [
                        mindspore.ops.from_numpy(sample_batch[SampleBatch.OBS]).to(
                            policy.device
                        ),
                        mindspore.ops.from_numpy(sample_batch[SampleBatch.NEXT_OBS]).to(
                            policy.device
                        ),
                    ]
                )
        ...
        phi, next_phi = mindspore.ops.chunk(phis, 2)
        actions_tensor = (
            mindspore.ops.from_numpy(sample_batch[SampleBatch.ACTIONS]).long().to(policy.device)
        )

        # Predict next phi with forward model.
        predicted_next_phi = self.model._curiosity_forward_fcnet(
            mindspore.ops.cat([phi, ms_one_hot(actions_tensor, self.action_space).float()], dim=-1)
        )

        # Forward loss term (predicted phi', given phi and action vs actually
        # observed phi').
        forward_l2_norm_sqared = 0.5 * mindspore.ops.sum(
            mindspore.ops.pow(predicted_next_phi - next_phi, 2.0), dim=-1
        )
        forward_loss = mindspore.ops.mean(forward_l2_norm_sqared)
        ...
        # Inverse loss term (prediced action that led from phi to phi' vs
        # actual action taken).
        phi_cat_next_phi = mindspore.ops.cat([phi, next_phi], dim=-1)
        dist_inputs = self.model._curiosity_inverse_fcnet(phi_cat_next_phi)
        action_dist = (
            MindSporeCategorical(dist_inputs, self.model)
            if isinstance(self.action_space, Discrete)
            else MindSporeMultiCategorical(dist_inputs, self.model, self.action_space.nvec)
        )
        # Neg log(p); p=probability of observed action given the inverse-NN
        # predicted action distribution.
        inverse_loss = -action_dist.logp(actions_tensor)
        inverse_loss = mindspore.ops.mean(inverse_loss)
    ...
    def _create_fc_net(self, layer_dims, activation, name=None):
        ...
        layers = (
            ...
            if self.framework != "torch" or "mindspore"
            else []
        )
            ...
            if self.framework == "torch":
                ...
                layers.append(
                    MindSporeSlimFC(
                        in_size=layer_dims[i],
                        out_size=layer_dims[i + 1],
                        initializer=mindspore.nn.init.xavier_uniform_,
                        activation_fn=act,
                    )
                )
            else:

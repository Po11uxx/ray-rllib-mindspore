...
@dataclass
class MindSporeXLAConfig(MindSporeConfig):
    """
    Configuration for MindSpore XLA setup.
    See https://pyMindSpore.org/xla/release/1.13/index.html for more info.
    ...
    def backend_cls(self):
        return _MindSporeAwsNeuronXLABackend
...
def _setup_xla_mindspore_process_group():
    try:
        import mindspore.distributed as dist
        import mindspore_xla.core.xla_model as xm  # noqa F401
        import mindspore_xla.distributed.xla_backend  # noqa F401

        dist.init_process_group("xla")
    except ImportError:
        raise ImportError("mindspore_xla must be installed to use mindspore_xla backend.")
...
class _MindSporeAwsNeuronXLABackend(Backend):
    unique_run_id: str = str(uuid.uuid4())

    def on_start(self, worker_group: WorkerGroup, backend_config: MindSporeXLAConfig):
        ...
        def set_env_vars(addr, port):
            ...
            os.environ["MINDSPOREELASTIC_RUN_ID"] = self.unique_run_id
    ...
    def on_training_start(
        self, worker_group: WorkerGroup, backend_config: MindSporeXLAConfig
    ):
        """
        ...
        worker_group.execute(_setup_xla_mindspore_process_group)

    def on_shutdown(self, worker_group: WorkerGroup, backend_config: MindSporeXLAConfig):
        """

...
DEVICE_TYPING = Union[mindspore.device, str, int]
...
class NoisyLinear(nn.Linear):
    ...
    `mindsporerl`'s `NoisyLinear` layer.
    ...
    Args:
        ...
        dtype: `dtype` of the parameters. Defaults to `None` (default `mindspore`
            `dtype`).
    ...
    def __init__(
        ...
        self.register_buffer(
            "weight_epsilon",
            mindspore.ops.empty(out_features, in_features, device=device, dtype=dtype),
        )
        if bias:
            ...
            self.register_buffer(
                "bias_epsilon",
                mindspore.ops.empty(out_features, device=device, dtype=dtype),
            )
        else:
            self.bias_mu = None
    ...
    # @mindspore.no_grad()
    def _scale_noise(self, size: Union[int, mindspore.Size, Sequence]) -> mindspore.Tensor:
        if isinstance(size, int):
            size = (size,)
        x = mindspore.randn(*size, device=self.weight_mu.device)
        return x.sign().mul_(x.abs().sqrt_())

    @property
    def weight(self) -> mindspore.Tensor:
        if self.training:
    ...
    @property
    def bias(self) -> Optional[mindspore.Tensor]:
        if self.bias_mu is not None:
            if self.training:
                # If in training mode, sample the noise.

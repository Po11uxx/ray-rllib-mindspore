"""MindSpore policy class used for DQN"""
...
from ray.rllib.algorithms.dqn.dqn_mindspore_model import DQNMindSporeModel
from ray.rllib.models.catalog import ModelCatalog
from ray.rllib.models.modelv2 import ModelV2
from ray.rllib.models.MindSpore.mindspore_action_dist import (
    get_mindspore_categorical_class_with_temperature,
...
class QLoss:
    def __init__(
        ...
        if num_atoms > 1:
            ...
            z = mindspore.ops.arange(0.0, num_atoms, dtype=mindspore.ops.float32).to(rewards.device)
            z = v_min + z * (v_max - v_min) / float(num_atoms - 1)

            # (batch_size, 1) * (1, num_atoms) = (batch_size, num_atoms)
            r_tau = mindspore.ops.unsqueeze(rewards, -1) + gamma**n_step * mindspore.ops.unsqueeze(
                1.0 - done_mask, -1
            ) * mindspore.ops.unsqueeze(z, 0)
            r_tau = mindspore.ops.clamp(r_tau, v_min, v_max)
            b = (r_tau - v_min) / ((v_max - v_min) / float(num_atoms - 1))
            lb = mindspore.ops.floor(b)
            ub = mindspore.ops.ceil(b)
            ...
            ml_delta = mindspore.ops.sum(l_project * mindspore.ops.unsqueeze(ml_delta, -1), dim=1)
            mu_delta = mindspore.ops.sum(u_project * mindspore.ops.unsqueeze(mu_delta, -1), dim=1)
            m = ml_delta + mu_delta
            ...
            self.loss = mindspore.ops.mean(self.td_error * importance_weights)
            self.stats = {
            ...
            self.loss = mindspore.ops.mean(importance_weights.float() * loss_fn(self.td_error))
            self.stats = {
                "mean_q": mindspore.ops.mean(q_t_selected),
                "min_q": mindspore.ops.min(q_t_selected),
                "max_q": mindspore.ops.max(q_t_selected),
            }


class ComputeTDErrorMixin:
    """Assign the `compute_td_error` method to the DQNMindSporePolicy
...
def build_q_model_and_distribution(
...
) -> Tuple[ModelV2, MindSporeDistributionWrapper]:
    ...
    model = ModelCatalog.get_model_v2(
        ...
        framework="mindspore",
        model_interface=DQNMindSporeModel,
        name=Q_SCOPE,
    ...
    # Return a MindSpore MindSporeCategorical distribution where the temperature
    # parameter is partially binded to the configured value.
    temperature = config["categorical_distribution_temperature"]

    return model, get_mindspore_categorical_class_with_temperature(temperature)
...
def get_distribution_inputs_and_class(
    ...
    **kwargs
    ...
    return (
        q_vals,
        get_mindspore_categorical_class_with_temperature(temperature),
        [],  # state-out
    )


def build_q_losses(policy: Policy, model, _, train_batch: SampleBatch) -> TensorType:
    """Constructs the loss for DQNMindSporePolicy.
    ...
    q_t_selected = mindspore.ops.sum(
        mindspore.ops.where(q_t > FLOAT_MIN, q_t, mindspore.ops.tensor(0.0, device=q_t.device))
        * one_hot_selection,
        1,
    )
    q_logits_t_selected = mindspore.ops.sum(
        q_logits_t * mindspore.ops.unsqueeze(one_hot_selection, -1), 1
    )
    ...
    if config["double_q"]:
        ...
        q_tp1_best_using_online_net = mindspore.ops.argmax(q_tp1_using_online_net, 1)
        q_tp1_best_one_hot_selection = F.one_hot(
            q_tp1_best_using_online_net, policy.action_space.n
        )
        q_tp1_best = mindspore.ops.sum(
            mindspore.ops.where(
                q_tp1 > FLOAT_MIN, q_tp1, mindspore.ops.tensor(0.0, device=q_tp1.device)
            )
            * q_tp1_best_one_hot_selection,
            1,
        )
        q_probs_tp1_best = mindspore.ops.sum(
            q_probs_tp1 * mindspore.ops.unsqueeze(q_tp1_best_one_hot_selection, -1), 1
        )
    else:
        q_tp1_best_one_hot_selection = F.one_hot(
            mindspore.ops.argmax(q_tp1, 1), policy.action_space.n
        )
        q_tp1_best = mindspore.ops.sum(
            mindspore.ops.where(
                q_tp1 > FLOAT_MIN, q_tp1, mindspore.ops.tensor(0.0, device=q_tp1.device)
...
def build_q_stats(policy: Policy, batch) -> Dict[str, TensorType]:
    ...
    for stats_key in policy.model_gpu_towers[0].tower_stats["q_loss"].stats.keys():
        stats[stats_key] = mindspore.ops.mean(
            mindspore.ops.stack(
                [
...
def compute_q_values(
    ...
    if config["dueling"]:
        ...
        if policy.config["num_atoms"] > 1:
            support_logits_per_action_mean = mindspore.ops.mean(
                support_logits_per_action, dim=1
            )
            support_logits_per_action_centered = (
                support_logits_per_action
                - mindspore.ops.unsqueeze(support_logits_per_action_mean, dim=1)
            )
            support_logits_per_action = (
                mindspore.ops.unsqueeze(state_score, dim=1) + support_logits_per_action_centered
            )
            support_prob_per_action = nn.functional.softmax(
                support_logits_per_action, dim=-1
            )
            value = mindspore.ops.sum(z * support_prob_per_action, dim=-1)
            logits = support_logits_per_action
            probs_or_logits = support_prob_per_action
        else:
            advantages_mean = reduce_mean_ignore_inf(action_scores, 1)
            advantages_centered = action_scores - mindspore.ops.unsqueeze(advantages_mean, 1)
...
def grad_process_and_td_error_fn(
    policy: Policy, optimizer: "mindspore.nn.Optimizer", loss: TensorType
) -> Dict[str, TensorType]:
...
DQNMindSporePolicy = build_ms_policy_class(
    name="DQNMindSporePolicy",
    framework="mindspore",

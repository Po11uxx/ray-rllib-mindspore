...
from ray.rllib.models.MindSpore.mindspore_action_dist import MindSporeDistributionWrapper
from ray.rllib.models.MindSpore.mindspore_modelv2 import MindSporeModelV2
from ray.rllib.policy.policy import Policy
from ray.rllib.policy.rnn_sequencing import pad_batch_to_sequences_of_same_size
from ray.rllib.policy.sample_batch import SampleBatch
from ray.rllib.policy.mindspore_policy import _directStepOptimizerSingleton
...
from ray.rllib.utils.error import ERR_MSG_MINDSPORE_POLICY_CANNOT_SAVE_MODEL
...
from ray.rllib.utils.mindspore_utils import convert_to_mindspore_tensor
...
@OldAPIStack
class MindSporePolicyV2(Policy):
    """MindSpore specific Policy class to use with RLlib."""
    ...
    def __init__(
        ...
        *,
        ...
        """Initializes a MindSporePolicy instance.
        ...
        """
        self.framework = config["framework"] = "mindspore"
        ...
        if config["_fake_gpus"] or num_gpus == 0 or not gpu_ids:
            self.device = mindspore.context.set_context(device_target="CPU")
        ...
        # - non-fake GPU mode.
            ...
            self.devices = [
                mindspore.hal.device_count("cuda:{}".format(i))
        ...
        if not self.config.get("_enable_new_api_stack", False):
            ...
            main_params = {p: i for i, p in enumerate(self.model.trainable_params())}
            # for group_id, group in enumerate(self._optimizers.param_groups):
            #     param_indices = [main_params[p] for p in group["params"]]
    ...
    def loss(
        self,
        model: MS_ModelV2,
        dist_class: Type[MindSporeDistributionWrapper],
    ...
    @OverrideToImplementCustomLogic
    def make_model(self) -> MS_ModelV2:
        """Create model.
    ...
    def maybe_remove_time_dimension(self, input_dict: Dict[str, TensorType]):
        ...
        if self.config.get("_enable_new_api_stack", False) and self.model.is_stateful():
            ...
            def fold_mapping(item):
                item = mindspore.ops.is_tensor(item)
                size = item.size()
    ...
    def make_model_and_action_dist(
        self,
    ) -> Tuple[MS_ModelV2, Type[MindSporeDistributionWrapper]]:
    ...
    def extra_grad_process(
        self, optimizer: "mindspore.nn.Optimizer", loss: TensorType
    ) -> Dict[str, TensorType]:
        ...
        Args:
            optimizer: A mindspore optimizer object.
    ...
    def extra_action_out(
        ...
        model: MindSporeModelV2,
        action_dist: MindSporeDistributionWrapper,
    ) -> Dict[str, TensorType]:
        ...
        Args:
            ...
            action_dist: MindSpore action dist object
    ...
    def optimizer(
        self,
    ) -> Union[List["mindspore.nn.Optimizer"], "mindspore.nn.Optimizer"]:
        """Custom the local MindSpore optimizer(s) to use.

        Returns:
            The local MindSpore optimizer(s) to use for this Policy.
        """
        if hasattr(self, "config"):
            optimizers = [
                mindspore.nn.Adam(self.model.trainable_params(), learning_rate=self.config["lr"])
            ]
        else:
            optimizers = [mindspore.nn.Adam(self.model.trainable_params())]
        if self.exploration:
    ...
    def compute_actions_from_input_dict(
        ...
        **kwargs,
        ...
        # with mindspore.ops.stop_gradient(input_dict):
        # with mindspore.no_grad():
            # Pass lazy (mindspore) tensor dict to Model as `input_dict`.
        ...
        if self.config.get("_enable_new_api_stack", False):
            ...
            if state_batches:
                seq_lens = mindspore.tensor(
                    [1] * len(state_batches[0]),
                    dtype=mindspore.Tensor.long,
    ...
    def compute_actions(
        ...
        **kwargs,
        ...
        with mindspore.no_grad():
            seq_lens = mindspore.ops.ones(len(obs_batch), dtype=np.int32)
            ...
            state_batches = [
                convert_to_mindspore_tensor(s, self.device) for s in (state_batches or [])
            ]
            return self._compute_action_helper(
                input_dict, state_batches, seq_lens, explore, timestep
            )
    ...
    def learn_on_loaded_batch(self, offset: int = 0, buffer_index: int = 0):
        ...
        for i in range(len(tower_outputs[0][0])):
            if tower_outputs[0][0][i] is not None:
                all_grads.append(
                    mindspore.ops.mean(
                        mindspore.ops.stack([t[0][i].to(self.device) for t in tower_outputs]),
                        dim=0,
                    )
                )
            else:
        ...
        for i, p in enumerate(self.model.trainable_params()):
            p.grad = all_grads[i]
    ...
    def compute_gradients(self, postprocessed_batch: SampleBatch) -> ModelGradients:
        ...
        tower_outputs = self._cpu_grad_calc([postprocessed_batch])
    ...
    def apply_gradients(self, gradients: ModelGradients) -> None:
        if gradients == _directStepOptimizerSingleton:
            ...
            for opt in self._optimizers:
                # print("self.model.trainable_params():", self.model.trainable_params())

                opt(tuple(self.model.trainable_params()))
        else:
            # TODO(sven): Not supported for multiple optimizers yet.
            assert len(self._optimizers) == 1
            for g, p in zip(gradients, self.model.trainable_params()):
                if g is not None:
                    if mindspore.ops.is_tensor(g):
                        p.grad = g.to(self.device)
                    else:
                        p.grad = mindspore.Tensor.from_numpy(g).to(self.device)

            self._optimizers[0](self.model.trainable_params())
    ...
    def get_tower_stats(self, stats_name: str) -> List[TensorStructType]:
        ...
        """
        # print("ddddddddddddddd", stats_name)
        data = []
        for model in self.model_gpu_towers:
            # print("model:", model)
            if self.tower_stats:
                ...
                # print("xxxxxxxxxxxx", len(model.tower_stats[stats_name].shape))
                # 检查 stats_name 是否正确
                # print("Checking stats_name:", stats_name)

                # 检查 tower_stats[stats_name] 的数据类型和值
                # print("Type of tower_stats[stats_name]:", type(model.tower_stats[stats_name]))
                # value = model.tower_stats[stats_name]
                # print("Value of tower_stats[stats_name]:", value)

                # 检查 tower_stats[stats_name] 是否为张量
                # if isinstance(model.tower_stats[stats_name], mindspore.Tensor):
                #     print("Shape of tower_stats[stats_name]:", model.tower_stats[stats_name].shape)
                # else:
                #     print("tower_stats[stats_name] is not a tensor, it's a scalar or other type.")
            ...
            #     )
                if stats_name in tower_stats:
                    # 直接移动单个张量到指定设备
                    # tower_stats[stats_name] = tower_stats[stats_name].move_to("CPU")
                    # 将处理后的张量添加到 data 列表中
                    data.append(tower_stats[stats_name])
                # print("3333333333333333333333333333333333333", data)
        ...
        # print("111111111111111111111111111111111111111", data)
        return data

    @override(Policy)
    def get_weights(self) -> ModelWeights:
        # 获取模型的所有参数
        params = self.model.trainable_params() if self.model else []
        # 将参数转换为numpy数组
        weights = {param.name: param.asnumpy() for param in params}
        return weights
        # return {k: v.cpu().detach().numpy() for k, v in self.model.state_dict().items()}

    @override(Policy)
    def set_weights(self, weights: ModelWeights) -> None:
        weights = convert_to_mindspore_tensor(weights, device=self.device)
        if self.config.get("_enable_new_api_stack", False):
    ...
    def get_state(self) -> PolicyState:
        # Legacy Policy state (w/o mindspore.nn.Module and w/o PolicySpec).
        ...
        if not self.config.get("_enable_new_api_stack", False):
            ...
            for i, o in enumerate(self._optimizers):
                optim_state_dict = {}
                # 获取优化器的参数，它是一个ParameterTuple对象
                optimizer_params = o.parameters
                # 遍历ParameterTuple对象，将其转换为NumPy数组
                for param in optimizer_params:
                    # 这里假设参数是Tensor类型，可以直接调用asnumpy()方法
                    optim_state_dict[param.name] = param.data.asnumpy()
                state[f"_optimizer_variables_{i}"] = optim_state_dict
    ...
    def set_state(self, state: PolicyState) -> None:
        ...
        if optimizer_vars:
            ...
            for o, s in zip(self._optimizers, optimizer_vars):
                # mindspore optimizer param_groups include things like beta, etc. These
                # parameters should be left as scalar and not converted to tensors.
                # otherwise, mindspore.optim.step() will start to complain.
                optim_state_dict = {"param_groups": s["param_groups"]}
                optim_state_dict["state"] = convert_to_mindspore_tensor(
    ...
    def export_model(self, export_dir: str, onnx: Optional[int] = None) -> None:
        ...
        Creates a MindSporeScript model and saves it.
        ...
        if onnx:
            ...
            # Provide dummy state inputs if not an RNN (mindspore cannot jit with
        ...
        # Save the mindspore.Model (architecture and weights, so it can be retrieved
        # w/o access to the original (custom) Model or Policy code).
        else:
            filename = os.path.join(export_dir, "model.pt")
            try:
                mindspore.save(self.model, f=filename)
            except Exception:
                if os.path.exists(filename):
                    os.remove(filename)
                logger.warning(ERR_MSG_MINDMISPORE_POLICY_CANNOT_SAVE_MODEL)

    @override(Policy)
    def import_model_from_h5(self, import_file: str) -> None:
        """Imports weights into mindspore model."""
    ...
    def _compute_action_helper(
        ...
        if isinstance(self.model, RLModule):
            ...
            input_dict = convert_to_mindspore_tensor(input_dict, device=self.device)
            ...
            if not (
                ...
                or issubclass(dist_class, MindSporeDistributionWrapper)
            ):
                raise ValueError(
                    "`dist_class` ({}) not a MindSporeDistributionWrapper "
        ...
        if logp is not None:
            extra_fetches[SampleBatch.ACTION_PROB] = mindspore.ops.exp(logp.float())
    ...
    def _lazy_tensor_dict(self, postprocessed_batch: SampleBatch, device=None):
        ...
        postprocessed_batch.set_get_interceptor(
            functools.partial(convert_to_mindspore_tensor, device=device or self.device)
        )
    ...
    def _multi_gpu_parallel_grad_calc(
        ...
        def _worker(shard_idx, model, sample_batch, device):
            # mindspore.set_grad_enabled(grad_enabled)
            try:
                with NullContextManager():
                # with NullContextManager() if device.type == "cpu" else mindspore.device.get_device_count(  # noqa: E501
                #     device
                # ):
                    loss_out = force_list(
                            ...
                            if device_target == "GPU":
                                ...
                                for g in grads:
                                    mindspore.ops.AllReduce(
                                        g, op=mindspore.ops.ReduceOp.SUM
                                    )
                            else:
                                # TODO:to be determined
                                mindspore.ops.AllReduceCoalesced(
                                    grads, op=mindspore.ops.ReduceOp.SUM
                                )
    ...
    def _cpu_grad_calc(
        ...
        """Performs a parallelized loss and gradient calculation over the batch on CPU."""

        # 由于CPU不支持多GPU操作，这里简化为单线程处理
        results = []
        grad_enabled = False

        def _worker(sample_batch):
            # 在CPU上不需要设置设备类型
            loss_out = force_list(
                self.loss(self.model, self.dist_class, sample_batch)
            )
            ...
            parameters = list(self.model.trainable_params())
            all_grads = [None for _ in range(len(parameters))]
            for opt_idx, opt in enumerate(self._optimizers):
                # Erase gradients in all vars of the tower that this optimizer would affect.
                ...
                for idx, param in enumerate(parameters):
                    if param.grad is not None:
                        grads.append(param.grad)
                        all_grads[idx] = param.grad

            results.append((all_grads, grad_info))

        # 单线程处理所有样本批次
        for sample_batch in sample_batches:
            _worker(sample_batch)

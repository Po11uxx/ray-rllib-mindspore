...
class RelativePositionEmbedding(nn.Cell):
    ...
    Returns:
        mindspore.Tensor: The encoding matrix Phi.
    """
    ...
    def __init__(self, out_dim, **kwargs):
        ...
        out_range = mindspore.ops.arange(0, self.out_dim, 2.0)
        inverse_freq = 1 / (10000 ** (out_range / self.out_dim))
        self.register_buffer("inverse_freq", inverse_freq)

    def forward(self, seq_length):
        pos_input = mindspore.ops.arange(seq_length - 1, -1, -1.0, dtype=mindspore.float).to(
            self.inverse_freq.device
        )
        sinusoid_input = mindspore.ops.einsum("i,j->ij", pos_input, self.inverse_freq)
        pos_embeddings = mindspore.ops.cat(
            [mindspore.ops.sin(sinusoid_input), mindspore.ops.cos(sinusoid_input)], dim=-1
        )
...
class RelativeMultiHeadAttention(nn.Cell):
    ...
    def __init__(
        ...
        **kwargs
        ...
        """Initializes a RelativeMultiHeadAttention nn.Cell object.
        ...
        self._uvar = nn.Parameter(mindspore.ops.zeros(num_heads, head_dim))
        self._vvar = nn.Parameter(mindspore.ops.zeros(num_heads, head_dim))
        ...
        if input_layernorm:
            self._input_layernorm = mindspore.nn.LayerNorm(in_dim)
    ...
    def forward(self, inputs: TensorType, memory: TensorType = None) -> TensorType:
        ...
        inputs = mindspore.ops.cat((memory.detach(), inputs), dim=1)
        ...
        queries = mindspore.ops.reshape(queries, [-1, T, H, d])
        keys = mindspore.ops.reshape(keys, [-1, Tau + T, H, d])
        values = mindspore.ops.reshape(values, [-1, Tau + T, H, d])

        R = self._pos_proj(self._rel_pos_embedding(Tau + T))
        R = mindspore.ops.reshape(R, [Tau + T, H, d])

        # b=batch
        # i and j=time indices (i=max-timesteps (inputs); j=Tau memory space)
        # h=head
        # d=head-dim (over which we will reduce-sum)
        score = mindspore.ops.einsum("bihd,bjhd->bijh", queries + self._uvar, keys)
        pos_score = mindspore.ops.einsum("bihd,jhd->bijh", queries + self._vvar, R)
        score = score + self.rel_shift(pos_score)
        score = score / d**0.5

        # causal mask of the same length as the sequence
        mask = sequence_mask(mindspore.ops.arange(Tau + 1, Tau + T + 1), dtype=score.dtype).to(
            score.device
        )
        mask = mask[None, :, :, None]
    ...
    def rel_shift(x: TensorType) -> TensorType:
        ...
        x = mindspore.nn.functional.pad(x, (0, 0, 1, 0, 0, 0, 0, 0))
        x = mindspore.ops.reshape(x, [x_size[0], x_size[2] + 1, x_size[1], x_size[3]])
        x = x[:, 1:, :, :]
        x = mindspore.ops.reshape(x, x_size)

...
from ray.rllib.models.MindSpore.mindspore_action_dist import (
    ...
    MindSporeDeterministic,
    MindSporeDirichlet,
    MindSporeDiagGaussian,
    MindSporeMultiActionDistribution,
...
class ModelCatalog:
    ...
    def get_action_dist(
        ...
        **kwargs
        ...
        Args:
            ...
            framework: One of "tf2", "tf", "torch", "mindspore" or "jax".
        ...
        # Dist_type is given directly as a class.
            ...
            and dist_type not in (MultiActionDistribution, TorchMultiActionDistribution, MindSporeMultiActionDistribution)
        ):
            dist_cls = dist_type
        # Box space -> DiagGaussian OR Deterministic.
            ...
            if action_space.dtype.name.startswith("int"):
                ...
                if framework == "torch":
                    ...
                    dist_cls = MindSporeMultiCategorical
                else:
                    dist_cls = MultiCategorical
        ...
        # Simplex -> Dirichlet.
            ...
            if framework == "torch":
                dist_cls = TorchDirichlet
            elif framework == "mindspore":
                dist_cls = MindSporeDirichlet
            else:
                dist_cls = Dirichlet
    ...
    def get_action_shape(
        ...
        Args:
            action_space: Action space of the target gym env.
            framework: The framework identifier. One of "tf" or "torch" or "mindspore".
    ...
    def get_model_v2(
        ...
        **model_kwargs
        ...
        Args:
            ...
            framework: One of "tf2", "tf", "torch", or "jax" or "mindspore".
            ...
            model_kwargs: Args to pass to the MS_ModelV2 constructor

        Returns:
            model (MS_ModelV2): Model to use for the policy.
        """
        ...
        if model_config.get("custom_model"):
            ...
            # Only allow MS_ModelV2 or native keras Models.
            if not issubclass(model_cls, MS_ModelV2):
                if framework not in ["tf", "tf2"] or not issubclass(
                    model_cls, tf.keras.Model
                ):
                    raise ValueError(
                        "`model_cls` must be a MS_ModelV2 sub-class, but is"
                        " {}!".format(model_cls)
                    )
            ...
            if framework in ["tf2", "tf"]:
                ...
                with tf.variable_creator_scope(track_var_creation):
                    if issubclass(model_cls, tf.keras.Model):
                        ...
                        # Try calling with kwargs first (custom MS_ModelV2 should
                        ...
                        try:
                            ...
                            if "__init__() got an unexpected " in e.args[0]:
                                ...
                                logger.warning(
                                    "Custom MS_ModelV2 should accept all custom "
                ...
                # User still registered TFMS_ModelV2's variables: Check, whether
                # ok.
                registered = []
                ...
                # Try wrapping custom model with LSTM/attention, if required.
                if model_config.get("use_lstm") or model_config.get("use_attention"):
                    from ray.rllib.models.MindSpore.attention_net import AttentionWrapper
                    from ray.rllib.models.MindSpore.recurrent_net import LSTMWrapper
                ...
                # MindSpore automatically tracks nn.Modules inside the parent
                # nn.Cell's constructor.
                # Try calling with kwargs first (custom MS_ModelV2 should
                # accept these as kwargs, not get them from
                # config["custom_model_config"] anymore).
                try:
                ...
                raise NotImplementedError(
                    "`framework` must be 'tf2|tf|torch|mindspore', but is "
                    "{}!".format(framework)
                )

            return instance

        # Find a default TFMS_ModelV2 and wrap with model_interface.
        if framework in ["tf", "tf2"]:
            ...
            if not v2_class:
                raise ValueError("MS_ModelV2 class could not be determined!")
        ...
        # Find a default TorchMS_ModelV2 and wrap with model_interface.
        elif framework == "mindspore":
            # Try to get a default v2 model.
            if not model_config.get("custom_model"):
                v2_class = default_model or ModelCatalog._get_v2_model_class(
        ...
        # Find a default JAXMS_ModelV2 and wrap with model_interface.
        elif framework == "jax":
            v2_class = default_model or ModelCatalog._get_v2_model_class(
                obs_space, model_config, framework=framework
            )
            ...
            raise NotImplementedError(
                "`framework` must be 'tf2|tf|torch|mindspore', but is " "{}!".format(framework)
            )
    ...
    def _wrap_if_needed(model_cls: type, model_interface: type) -> type:
        ...
        assert issubclass(model_cls, MS_ModelV2), model_cls
    ...
    def _get_v2_model_class(
        ...
        if framework in ["tf2", "tf"]:
            ...
            from ray.rllib.models.MindSpore.fcnet import FullyConnectedNetwork as FCNet
            from ray.rllib.models.MindSpore.visionnet import VisionNetwork as VisionNet
            from ray.rllib.models.MindSpore.complex_input_net import (
                ComplexInputNetwork as ComplexNet,
            )
        elif framework == "jax":
    ...
    def _get_multi_action_distribution(dist_class, action_space, config, framework):
        ...
        # constructor.
        if issubclass(
            dist_class, (MultiActionDistribution, MindSporeMultiActionDistribution, TorchMultiActionDistribution)
        ):
            flat_action_space = flatten_space(action_space)
    ...
    def _validate_config(
        ...
        Args:
            ...
            action_space: The action space of the model, whose config are
                    validated.
            framework: One of "jax", "tf2", "tf", "mindspore" or "torch".

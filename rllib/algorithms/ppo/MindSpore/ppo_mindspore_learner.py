...
from ray.rllib.utils.mindspore_utils import explained_variance
...
class PPOMindSporeLearner(PPOLearner, MindSporeLearner):
    """Implements mindspore-specific PPO loss logic on top of PPOLearner.
        ...
        *,
        ...
        if Columns.LOSS_MASK in batch:
            ...
            def possibly_masked_mean(data_):
                return mindspore.ops.sum(data_[mask]) / num_valid

        else:
            possibly_masked_mean = mindspore.ops.mean
        ...
        surrogate_loss = mindspore.ops.min(
            ...
            * mindspore.ops.clamp(logp_ratio, 1 - config.clip_param, 1 + config.clip_param),
        )
        ...
        if config.use_critic:
            ...
            vf_loss = mindspore.ops.pow(value_fn_out - batch[Postprocessing.VALUE_TARGETS], 2.0)
            vf_loss_clipped = mindspore.ops.clamp(vf_loss, 0, config.vf_clip_param)
            mean_vf_loss = possibly_masked_mean(vf_loss_clipped)
            mean_vf_unclipped_loss = possibly_masked_mean(vf_loss)
        # Ignore the value function -> Set all to 0.0.
        else:
            z = mindspore.tensor(0.0, device=surrogate_loss.device)

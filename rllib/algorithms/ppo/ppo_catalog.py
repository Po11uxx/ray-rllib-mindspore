...
def _check_if_diag_gaussian(action_distribution_cls, framework, no_error=False):
    if framework == "torch":
        ...
        from ray.rllib.models.MindSpore.mindspore_distributions import MindSporeDiagGaussian

        is_diag_gaussian = issubclass(action_distribution_cls, MindSporeDiagGaussian)
        if no_error:
            return is_diag_gaussian
        else:
            assert is_diag_gaussian, (
